...
- Window XP
- Opnet 11.5.A PL 3 (Model Library:  11.5 (10 APR 2006))
- MS Visual C++.Net (2002)

2/ The Opnet compiling flags and binding flags shall be set as shown in the env_db11.5_sample file 
...
Update: As of 8/23/2006, a complete build that supports all NRL Opnet code requires the 'opnet/11.5' and 'common' subdirectories from the following set of directories:
	smf
	protolib
	nrlolsr
	norm
	mgen
	mdp
In addition mdp/win32 is required.
These directories should be placed in a common parent directory (eg., C:/op_models/nrl).


WHAT TO DO AFTER DOWNLOADING 

1/  Opnet only reconizes external files that are ended with .ex.cpp or .ex.c.  Thus, the user will need to convert the file names
of "cpp" files in all the */common directories to have ".ex.cpp" extensions. For example, change nrlolsr.cpp to nrlolsr.ex.cpp
...
Update: A new .bat file (rename_cpp.bat) is included in the nrlolsr/opnet directory that converts a subset of .cpp files
to .ex.cpp files in the 'common' subdirectories of 'smf', 'nrlolsr', 'protolib', 'norm', 'mgen', and 'mdp'.
Execute rename_cpp.bat from its existing location in nrlolsr/opnet. The reverse operation, i.e., converting .ex.cpp files
back to .cpp files may be performed by calling undo_rename_cpp.bat.  These scripts operate explicitly on the
files required by the Opnet process models, which is only a subset of the .cpp files included in these directories.
...
Update: env_db11.5_sample.txt provides an up-to-date (as of 8/23/2006) sample Opnet environment file.
...
Update: 'udp_gen' has been replaced by 'mgen' which supports both UDP and TCP traffic sources and sinks.  The mgen.prj project contains a variety of scenarios that illustrate various ways to configure mgen as well as norm and mdp.  nrlolsr, smf, and mgen are present in all node models.
...
Opnet ip_dispatch process model configuration:
	It may be necessary to extend the list of allowable ip module routing protocols by adding "OLSR_NRL".  This is done by editing the ip_dispatch or ip_dispatch_smf process model/Interfaces/Model Attributes/ip router parameters(Edit Properties)/Interface Information(Edit Properties)/Routing Protocols(Edit Properties).
