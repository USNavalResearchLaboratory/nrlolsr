...
Update: As of 8/23/2006, a complete build that supports all NRL Opnet code requires the 'opnet' and 'common' subdirectories from the following set of directories:
	smf
	protolib
	nrlolsr
	norm
	mgen
	mdp
These directories should be placed in a common parent directory (eg., C:\op_models\11.5.A).
...
Update: A new .bat file (rename_cpp.bat) is included in the nrlolsr/opnet directory that converts all .cpp files to .ex.cpp files in the 'common' subdirectories of 'smf', 'nrlolsr', 'protolib', 'norm', 'mgen', and 'mdp'.  Execute rename_cpp.bat from its existing location in nrlolsr/opnet. 
