...
/* Maximum length of a dest_src_key 									*/
/* 4 bytes src fast addr, 4 bytes dest fast addr, 4 bytes lookup_index	*/
#define IPC_DEST_SRC_KEY_LEN_BYTES	12
#define IPC_DEST_SRC_KEY_LEN_INTS	3
...
static int			IpC_Cmn_Rte_Table_Hash_Size_Log = 5;
...
typedef struct IpT_Cmn_Rte_Dest_Src_Table_Entry
	...
	/* Store the key for debugging purposes. No way to retrieve the key	*/
	/* from the table in bin_hash.										*/
...
static IpT_Redist_Info *
ip_cmn_rte_table_redist_info_search (IpT_Redist_Matrix_Entry *redist_matrix_entry, IpT_Rte_Proc_Id routeproc_id, InetT_Addr_Family addr_family);
...
static void
ip_cmn_rte_table_hash_key_create (IpT_Cmn_Rte_Dest_Src_Table_Key key_ptr, int src_fast_addr, int dest_fast_addr, int lookup_index);

static Compcode
ip_cmn_rte_table_fast_addrs_from_hash_key_get (IpT_Cmn_Rte_Dest_Src_Table_Key key_ptr,
	int* src_fast_addr_ptr, int* dest_fast_addr_ptr, int* lookup_index_ptr);

#ifdef IP_DEST_SRC_TABLE

static void
ip_cmn_rte_table_dest_src_table_entry_print (IpT_Cmn_Rte_Dest_Src_Table_Entry* dest_src_entry_ptr);
...
#define ip_cmn_rte_table_dest_src_table_lookup(_dst_src_tbl, _key)	((IpT_Cmn_Rte_Dest_Src_Table_Entry*) prg_bin_hash_table_item_get ((_dst_src_tbl), (_key)))
#define ip_cmn_rte_table_dest_src_table_create(_rte_table)			(prg_bin_hash_table_create (IpC_Cmn_Rte_Table_Hash_Size_Log, IPC_DEST_SRC_KEY_LEN_BYTES))
...
ip_cmn_rte_table_create (Objid node_objid, IpT_Rte_Module_Data* ip_rmd_ptr,
	...
	ip_rte_map_callbacks_register (IPC_DYN_RTE_DIRECTLY_CONNECTED,
				(IpT_Rte_Map_Entry_Match_Proc) ip_cmn_rte_table_dir_conn_rte_entry_match,
				OPC_NIL /* no set proc */);

	/* Register match procs for IOS XR RPL entries for direct and static.	*/
	/* Set procs are not required since static and direct routes cannot be	*/
	/* manipulated using policies.											*/
	ip_rte_rpl_callbacks_register (IPC_DYN_RTE_DIRECTLY_CONNECTED,
		ip_rte_rpl_dir_conn_entry_match, OPC_NIL);

	ip_rte_print_proc_register (IPC_DYN_RTE_DIRECTLY_CONNECTED,
		(IpT_Rte_Entry_Print_Proc) ip_cmn_rte_table_entry_print);
...
#ifdef OPD_PARALLEL
	/* Initialize spinlock */
	route_table->ip_cmn_rte_table_spinlock = PRGC_MT_SPINLOCK_INIT;
#endif /* OPD_PARALEL */
...
Inet_Cmn_Rte_Table_Entry_Add_Options (IpT_Cmn_Rte_Table* route_table,	void* src_obj_ptr,
	...
	prg_mt_spinlock_acquire (&route_table->ip_cmn_rte_table_spinlock);
	...
	if (OPC_COMPCODE_FAILURE == ip_cmn_rte_table_dest_prefix_verify (route_table, dest_prefix))
		{
		prg_mt_spinlock_release (&route_table->ip_cmn_rte_table_spinlock);
		FRET (OPC_COMPCODE_FAILURE);
		}

	/* Check if addition for indirect nexthop option is set */
	...
	if ((proto_type < IPC_INITIAL_CUSTOM_RTE_PROTOCOL_ID) && (proto_type != IPC_DYN_RTE_DEFAULT))
		...
		if (PRGC_NIL != curr_ptree_entry)
			...
			if ((curr_entry->admin_distance == admin_distance) && (curr_entry->route_src_proto == proto))
				...
				/* RK: SPR-78120. Update source entry as well.	*/
				curr_entry->route_src_obj_ptr = src_obj_ptr;
	...
	if (proto_type != IPC_DYN_RTE_LOCAL)
		{
...
Inet_Cmn_Rte_Table_Clear (IpT_Cmn_Rte_Table* route_table, InetT_Addr_Family addr_family)
	...
	/* Destroy the hash table, but do not destroy the values in it */
         if (route_table->dest_src_table)
		prg_bin_hash_table_destroy (route_table->dest_src_table, ip_cmn_rte_table_dest_src_tbl_entry_free);
...
ip_cmn_rte_default_route_list_add (IpT_Cmn_Rte_Table* cmn_rte_table,
	...
	for (entry_index = 0; entry_index < num_entries; entry_index++)
		...
		/* the same.										*/
		/* The admin weight must also be the same. In case	*/
		/* of static routes, different next hops can have	*/
		/* different weights. These are considered different*/
		/* routes.											*/			
		...
		if ((ip_cmn_rte_table_dest_prefix_equal
			...
			(ith_list_entry->admin_distance == (*new_entry_pptr)->admin_distance))
			{
...
ip_cmn_rte_table_new_default_route_handle (IpT_Cmn_Rte_Table* cmn_rte_table,
	...
	/* best default route, make it the best default route.				*/
				...
				((OPC_NIL == cmn_rte_table->best_default_route) || 
...
ip_cmn_rte_table_rte_list_entry_delete (IpT_Cmn_Rte_Table* route_table, IpT_Cmn_Rte_Table_Entry* route_entry,
	...
	if ((OPC_NIL != route_entry->backup_list) && (op_prg_list_size (route_entry->backup_list) > 0))
		...
		if (generic_rte_info_ptr == OPC_NIL)
			...
			/* RIPng, BGP, Static, OSPF and ISIS use inet_install	*/
			/* proc, others use the install_proc.					*/
			if ((IPC_DYN_RTE_STATIC != backup_proto_type) &&
				(IPC_DYN_RTE_RIPNG != backup_proto_type) &&
				(IPC_DYN_RTE_BGP != backup_proto_type) &&
				(IPC_DYN_RTE_ISIS != backup_proto_type) &&
...
inet_cmn_rte_table_lookup_cache (IpT_Cmn_Rte_Table* route_table, InetT_Address dest, 
	...
	int							dest_src_tbl_key[IPC_DEST_SRC_KEY_LEN_INTS];
	...
	if ((route_table->load_type == IpC_Rte_Table_Load_Dest) && 
		...
		if (OPC_NIL != dest_src_entry_ptr)
			...
			if (route_table->last_update_time > dest_src_entry_ptr->creation_time)
				...
				if (best_entry_ptr != dest_src_entry_ptr->route_entry_ptr)
					...
					ip_cmn_rte_table_dest_src_key_list_entry_add
						(dest_src_entry_ptr->next_hop_ptr, (int*) prg_mem_copy_create (dest_src_tbl_key, IPC_DEST_SRC_KEY_LEN_BYTES));
					}
				}
			else
				{
			...
			if (OPC_NIL != best_entry_ptr)
				...
				/* chosen next hop.									*/
				ip_cmn_rte_table_dest_src_key_list_entry_add
					(next_hop_ptr, (int*) prg_mem_copy_create (dest_src_tbl_key, IPC_DEST_SRC_KEY_LEN_BYTES));
				}
			else
				{
...
ip_cmn_route_table_ptree_lookup (IpT_Cmn_Rte_Table* route_table, InetT_Address dest)
	...
	ptree_entry = (ptree_ptr != OPC_NIL) ? oms_ptree_address_lookup (ptree_ptr, address): OPC_NIL;
...
ip_cmn_rte_table_dest_src_table_entry_add (IpT_Cmn_Rte_Table* route_table_ptr,
	...
	prg_bin_hash_table_item_insert (route_table_ptr->dest_src_table,
		key, dest_src_table_entry, PRGC_NIL);

#ifdef IP_DEST_SRC_TABLE
	dest_src_table_entry->key = prg_mem_copy_create (key, IPC_DEST_SRC_KEY_LEN_BYTES);
#endif	
...
ip_cmn_rte_table_dest_src_table_entry_remove (void* route_table_void_ptr, void* key_void_ptr)
	...
	key = (IpT_Cmn_Rte_Dest_Src_Table_Key) key_void_ptr;
	
	/* Remove the corresponding entry from the hash table.		*/
	prg_bin_hash_table_item_remove (route_table_ptr->dest_src_table, key);
...
inet_cmn_rte_table_entry_exists (IpT_Cmn_Rte_Table* route_table, IpT_Dest_Prefix dest_prefix,
	...
	if ((0 == ip_cmn_rte_table_dest_prefix_mask_len_get (dest_prefix)) &&
		...
		/* Check whether the entry actually exists.					*/
		index = (OPC_NIL == ptree_ptr) ? OMSC_PTREE_ENTRY_INDEX_INVALID :
					oms_ptree_entry_exists (ptree_ptr, address,
						ip_cmn_rte_table_dest_prefix_mask_len_get (dest_prefix));
	...
	for (i=0; i < num_routes; i++)
		...
		if ((OPC_NIL != next_hop_ptr->table_key_lptr) && (op_prg_list_size (next_hop_ptr->table_key_lptr) > 0))
			...
			for (j = 0; j < num_keys; j++)
				{
				key = (IpT_Cmn_Rte_Dest_Src_Table_Key) op_prg_list_access (next_hop_ptr->table_key_lptr, j);
...
ip_cmn_rte_table_entry_create (IpT_Dest_Prefix dest_prefix, IpT_Rte_Proc_Id src_proto,
	...
	/* A local copy of the dest prefix must be created. IP	*/
	/* owns memory related to common route table entry.		*/
	route_entry->dest_prefix		= inet_address_range_copy (dest_prefix);
...
ip_cmn_rte_table_entry_copy (IpT_Cmn_Rte_Table_Entry* entry_ptr)
	...
	new_entry_ptr->dest_prefix			= inet_address_range_ptr_copy (&entry_ptr->dest_prefix);
...
ip_cmn_rte_table_entry_free (IpT_Cmn_Rte_Table_Entry* entry, IpT_Cmn_Rte_Table* route_table)
	...
	/* Destination prefix memory is owned by IP.	*/
	inet_address_range_destroy (&entry->dest_prefix);
...
static void 
ip_cmn_rte_table_dest_src_tbl_entry_free (void* void_entry_ptr)
	{
	IpT_Cmn_Rte_Dest_Src_Table_Entry* entry_ptr;
	/* This function is called to free the entry of dest_src_table */
	FIN (ip_cmn_rte_table_dest_src_tbl_entry_free (void*));
	
	entry_ptr = (IpT_Cmn_Rte_Dest_Src_Table_Entry *)void_entry_ptr;

#ifdef IP_DEST_SRC_TABLE
	op_prg_mem_free (entry_ptr->key);
#endif
...
ip_cmn_rte_table_redistribute (IpT_Cmn_Rte_Table* cmn_rte_table)
	...
	/* Go through IPv4 and IPv6 tables and redistribute		*/
	/* directly connected routes. The following loop		*/
	/* assumes that the indices of the two tables are 0 	*/
	/* and 1.												*/
	for (table_index = 0; table_index < 2; table_index++)
		{
		ptree_ptr = cmn_rte_table->ptree_ptr_array[table_index];

		if (OPC_NIL == ptree_ptr)
			{
			/* This version is not enabled on this node.	*/
			FOUT;
			}
...
ip_cmn_rte_table_entry_redistribute (IpT_Cmn_Rte_Table* cmn_rte_table, IpT_Cmn_Rte_Table_Entry* route_ptr,
	...
	if (redist_type == IPC_REDIST_TYPE_UPDATE_DIRECT)
			...
			if ((from_redist_matrix_entry != OPC_NIL) && (num_backups == 0))
				...
				for (j = 0; j < prg_list_size (from_redist_matrix_entry->redist_routeproc_lptr); j++)
					...
					if ((to_redist_info->routeproc_id == dest_route_proc_info_ptr->routeproc_id) &&
						ip_redist_addr_family_is_set (to_redist_info,
								ip_cmn_rte_table_dest_prefix_addr_family_get (route_copy_ptr->dest_prefix)))
						{
		...
		for (i = 0; i < prg_list_size (redist_routeproc_lptr); i++)
			...
			/* Make sure that the address family of the route	*/
			/* is the one requested by the target protocol. For	*/
			/* example, BGP may have requested only IPv6 static	*/
			/* or connected routes.								*/
			if (!ip_redist_addr_family_is_set (to_redist_info, 
				ip_cmn_rte_table_dest_prefix_addr_family_get (route_ptr->dest_prefix)))
				continue;
...
ip_cmn_rte_table_prune_backups_for_redistribution (IpT_Cmn_Rte_Table *cmn_rte_table, IpT_Cmn_Rte_Table_Entry *route_ptr,
	...
	for (i = 0; i < orig_list_size; i++)
		...
		for (j = 0; j < prg_list_size (from_redist_matrix_entry->redist_routeproc_lptr); j++)
			...
			/* If this protocol is not interested in this addr	*/
			/* family, then skip this entry.					*/
			if (!ip_redist_addr_family_is_set (to_redist_info,	
				ip_cmn_rte_table_dest_prefix_addr_family_get (route_ptr->dest_prefix)))
				continue;
...
ip_cmn_rte_table_redist_matrix_entries_combine (IpT_Redist_Matrix_Entry *in_table_redist_matrix_entry,
	...
	for (i = 0; i < op_prg_list_size (removed_redist_matrix_entry->redist_routeproc_lptr); i++)
		...
		if (!can_be_ignored)
			...
			/* Transfer the flags also (supported address family etc).	*/
			temp_redist_info->flags = old_redist_info->flags;
...
ip_cmn_rte_global_exp_file_create (void)
	...
	if (last_time < op_sim_time ())
		...
		model_name = ip_run_name_sim_attr_get (OPC_TRUE);
...
void
Inet_Cmn_Rte_Table_Install_Redist_Matrix_Entry (IpT_Cmn_Rte_Table *ip_route_table, IpT_Rte_Proc_Id dest_routeproc_id,
											  IpT_Rte_Proc_Id src_routeproc_id, void *redist_metric, int bgp_redist_type,
											  InetT_Addr_Family addr_family, void* rte_map_entry_ptr)
	...
	/* dest protocol.  This will be inserted into	*/
	/* the redistribution matrix.					*/
	temp_redist_info_ptr = ip_cmn_rte_table_redist_info_create (dest_routeproc_id, redist_metric, bgp_redist_type);
	if (temp_redist_info_ptr == OPC_NIL)
		FOUT;

	/* Note requested address family. This is required to filter updates	*/
	/* of unwanted address families. For example, BGP supports multiple 	*/
	/* address families, but RIPng only cares about IPv6.					*/
	ip_redist_addr_family_set (temp_redist_info_ptr, addr_family);

	temp_redist_info_ptr->rte_map_entry_ptr = rte_map_entry_ptr;
...
void *
Inet_Cmn_Rte_Table_Redist_Metric_Get (IpT_Cmn_Rte_Table *ip_route_table, IpT_Rte_Proc_Id to_routeproc_id, IpT_Rte_Proc_Id from_routeproc_id, InetT_Addr_Family addr_family)
	...
	FIN (Inet_Cmn_Rte_Table_Redist_Metric_Get (ip_route_table, to_routeproc_id, from_routeproc_id, addr_family));
	...
	/* to_routeproc_id.										*/
	temp_redist_info = ip_cmn_rte_table_redist_info_search (temp_redist_matrix_entry, to_routeproc_id, addr_family);
...
void *
Inet_Cmn_Rte_Table_Redist_Rte_Map_Get (IpT_Cmn_Rte_Table *ip_route_table, IpT_Rte_Proc_Id to_routeproc_id, IpT_Rte_Proc_Id from_routeproc_id, InetT_Addr_Family addr_family)
	...
	FIN (Inet_Cmn_Rte_Table_Redist_Rte_Map_Get (ip_route_table, to_routeproc_id, from_routeproc_id, addr_family));
	...
	/* Return the redistribution metric for the correct process	*/
	FRET (temp_redist_info->rte_map_entry_ptr);
	}
...
void
inet_cmn_rte_table_redist_proto_add_message (IpT_Rte_Proc_Id dest_routeproc_id, IpT_Rte_Proc_Id src_routeproc_id,
										   InetT_Address_Range* dest_prefix, InetT_Address next_hop, int metric)
	{
	char	dest_proto_str [64], src_proto_str [64];
	char	dest_str [INETC_ADDR_RANGE_STR_LEN], next_hop_str [INETC_ADDR_STR_LEN];
	char	trace_msg [512], trace_msg2 [512];
	...
	FIN (inet_cmn_rte_table_redist_proto_add_message (dest_routeproc_id, src_routeproc_id, dest_addr,next_hop, metric));
	
	ip_cmn_rte_proto_name_print (dest_proto_str, dest_routeproc_id);
	ip_cmn_rte_proto_name_print (src_proto_str, src_routeproc_id);
	inet_address_range_print (dest_str, dest_prefix);
	inet_address_print (next_hop_str, next_hop);
			
	sprintf (trace_msg, "The following redistributed route is being added to the route table of %s", dest_proto_str);
	sprintf (trace_msg2, "Protocol: %s  Dest: %s  Next Hop: %s  Metric: %d", src_proto_str, dest_str, next_hop_str, metric);
...
void
inet_cmn_rte_table_redist_proto_withdraw_message (IpT_Rte_Proc_Id dest_routeproc_id, IpT_Rte_Proc_Id src_routeproc_id,
	...
	FIN (inet_cmn_rte_table_redist_proto_withdraw_message (dest_routeproc_id, src_routeproc_id, dest_addr, next_hop));
	...
	sprintf (trace_msg, "The following redistributed route is being withdrawn from the route table of %s", dest_proto_str);
	sprintf (trace_msg2, "Protocol: %s  Dest: %s  Next Hop: %s", src_proto_str, dest_str, next_hop_str);
...
void
inet_cmn_rte_table_redist_proto_update_message (IpT_Rte_Proc_Id dest_routeproc_id, IpT_Rte_Proc_Id src_routeproc_id,
											  InetT_Address_Range* dest_prefix, InetT_Address next_hop, int metric)
	{
	char						dest_proto_str [64], src_proto_str [64];
	char	dest_str [INETC_ADDR_RANGE_STR_LEN], next_hop_str [INETC_ADDR_STR_LEN];
	...
	FIN (ip_cmn_rte_table_redist_proto_update_message (dest_routeproc_id, src_routeproc_id, dest_addr, next_hop, metric));
	...
	sprintf (trace_msg, "The following redistributed route is being updated in the route table of %s", dest_proto_str);
	sprintf (trace_msg2, "New Information:");
	sprintf (trace_msg3, "Protocol: %s  Dest: %s  Next Hop: %s  Metric: %d", src_proto_str, dest_str, next_hop_str, metric);
...
ip_cmn_rte_table_redist_info_search (IpT_Redist_Matrix_Entry *redist_matrix_entry, IpT_Rte_Proc_Id routeproc_id, InetT_Addr_Family addr_family)
	...
	FIN (ip_cmn_rte_table_redist_info_search (redist_matrix_entry, routeproc_id, addr_family));
	...
	for (i = 0; i < op_prg_list_size (redist_matrix_entry->redist_routeproc_lptr); i ++)
		...
		if ((temp_redist_info->routeproc_id == routeproc_id) && 
			ip_redist_addr_family_is_set (temp_redist_info, addr_family))
			FRET (temp_redist_info);
		}
...
static void
ip_cmn_rte_table_hash_key_create (int* key_ptr, int src_fast_addr, int dest_fast_addr, int lookup_index)
	{
	...
	key_ptr [0] = src_fast_addr;
	key_ptr [1] = dest_fast_addr;
	key_ptr [2] = lookup_index;
...
ip_cmn_rte_table_fast_addrs_from_hash_key_get (IpT_Cmn_Rte_Dest_Src_Table_Key key_ptr,
	...
	*src_fast_addr_ptr = key_ptr [0];
	*dest_fast_addr_ptr = key_ptr [1];
	*lookup_index_ptr = key_ptr [2];
...
ip_cmn_rte_table_dest_src_table_print (IpT_Cmn_Rte_Table* route_table_ptr)
	{
	PrgT_List*			hash_table_item_list;
	int					i, num_entries;
	...
	hash_table_item_list = prg_bin_hash_table_item_list_get (route_table_ptr->dest_src_table);
	
	/* Get the number of keys.								*/
	num_entries = prg_list_size (hash_table_item_list);

	for (i = 0; i < num_entries; i++)
		{
		/* Get the corresponding entry.						*/
		ith_entry_ptr = (IpT_Cmn_Rte_Dest_Src_Table_Entry*)
			prg_list_remove (hash_table_item_list, PRGC_LISTPOS_HEAD);

#ifdef IP_DEST_SRC_TABLE		
		/* Print the current entry.							*/
		ip_cmn_rte_table_dest_src_table_entry_print (ith_entry_ptr);
#else
		printf ("Please recompile ip_cmn_rte_table.ex.c with IP_DEST_SRC_TABLE defined\n");
#endif
		}
...
static void
ip_cmn_rte_table_dest_src_table_entry_print (
	IpT_Cmn_Rte_Dest_Src_Table_Entry* dest_src_entry_ptr)
	{
	...
	/** This function can be executed only when this file is compiled	**/
	/** with IP_DEST_SRC_TABLE. Otherwise, the key is not available in	**/
	/** the data structure.												**/

	/** Print the contents of the given entry.				**/

	FIN (ip_cmn_rte_table_dest_src_table_entry_print (dest_src_entry_ptr));

	
	/* Extract the source and Destination fast addresses	*/
	/* from the key string.									*/
	ip_cmn_rte_table_fast_addrs_from_hash_key_get (dest_src_entry_ptr->key,
		&src_fast_addr, &dest_fast_addr, &lookup_index);
...
void
ip_cmn_rte_table_redist_matrix_print (IpT_Cmn_Rte_Table* rte_table_ptr)
	{
	int		num_src_protos, src_proto_index, num_dest_protos, dest_proto_index;
	char	proto_str [128];
	IpT_Redist_Matrix_Entry*	src_info_ptr;
	IpT_Redist_Info*			dest_info_ptr;
	
	/** Print the contents of the redistribution matrix, for debugging purposes.	**/	
	FIN (ip_cmn_rte_table_redist_matrix_print ());

	num_src_protos = prg_vector_size (rte_table_ptr->redist_matrix_vptr);

	for (src_proto_index = 0; src_proto_index < num_src_protos; src_proto_index++)
		{
		src_info_ptr = (IpT_Redist_Matrix_Entry *) prg_vector_access (rte_table_ptr->redist_matrix_vptr, src_proto_index);
		ip_cmn_rte_proto_name_print (proto_str, src_info_ptr->routeproc_id);
		printf ("\n%s -", proto_str);

		num_dest_protos = op_prg_list_size (src_info_ptr->redist_routeproc_lptr);

		for (dest_proto_index = 0; dest_proto_index < num_dest_protos; dest_proto_index++)
			{
			dest_info_ptr = (IpT_Redist_Info *) op_prg_list_access (src_info_ptr->redist_routeproc_lptr, dest_proto_index);
			ip_cmn_rte_proto_name_print (proto_str, dest_info_ptr->routeproc_id);
			printf ("\t%s (%x)", proto_str, dest_info_ptr->flags);
			}
		}
	FOUT;
	}
...
DciT_Policy_Ret_Code 
ip_rte_rpl_dir_conn_entry_match (void* target_struct, DciT_Policy_Property property_type, 
	 DciT_Policy_Match_Condition PRG_ARG_UNUSED (condition_type), PrgT_List* values_lptr)
	{
	Boolean						match = OPC_FALSE;
	IpT_Cmn_Rte_Table_Entry*	rte_entry_ptr;
	IpT_Rte_RPL_State*			rpl_state_ptr;
	IpT_Next_Hop_Entry*			next_hop_ptr;
	
	FIN (ip_rte_rpl_dir_conn_entry_match ());

	rpl_state_ptr = (IpT_Rte_RPL_State *) target_struct;
	rte_entry_ptr = (IpT_Cmn_Rte_Table_Entry *) rpl_state_ptr->orig_entry_ptr;

	switch (property_type)
		{
		case DciC_Policy_Property_Destination:
			match = ip_rte_rpl_prefix_match (&rte_entry_ptr->dest_prefix, values_lptr);
			break;
			
		case DciC_Policy_Property_Next_Hop:
			/* We will use the first next-hop for matching purposes.	*/
			next_hop_ptr = (IpT_Next_Hop_Entry*) op_prg_list_access
					(rte_entry_ptr->next_hop_list, OPC_LISTPOS_HEAD);

			match = ip_rte_rpl_address_match (&next_hop_ptr->next_hop, values_lptr);
			break;

		default:
			/* Other properties are invalid for direct routes, or are not supported in DES.	*/
			; /* Some compilers do not like empty cases (in opt kernel).	*/
#ifndef OPD_NO_DEBUG
			if (op_prg_odb_ltrace_active ("rte_policy"))
				{
				char	msg_str [1024];
				strcpy (msg_str, "The property: ");
				Ip_Rte_RPL_Property_To_String (msg_str, property_type);
			   	strcat (msg_str, " is either invalid for dir-conn routes, or is not supported in DES.");
				op_prg_odb_print_major (msg_str, OPC_NIL);
				}
#endif
		}	

	FRET (match ? DciC_Policy_Ret_Code_Success : DciC_Policy_Ret_Code_Failure);
	}
			
Boolean				
ip_cmn_rte_table_process_redistributes_direct (IpT_Cmn_Rte_Table* route_table, IpT_Rte_Proc_Id proc_id, InetT_Addr_Family addr_family)
	{
	Boolean						redistributes_direct = OPC_FALSE;
	IpT_Redist_Matrix_Entry*	direct_matrix_entry_ptr;
	IpT_Rte_Proc_Id				direct_proto_id = IP_CMN_RTE_TABLE_UNIQUE_ROUTE_PROTO_ID (IPC_DYN_RTE_DIRECTLY_CONNECTED, IPC_NO_MULTIPLE_PROC);	
	
	FIN (ip_cmn_rte_table_process_redistributes_direct ());

	/* First check if anybody redistributes direct. Go through the matrix and find the 	*/
	/* entry for directly-connected.													*/		
	direct_matrix_entry_ptr = ip_cmn_rte_table_redist_matrix_entry_search (route_table, direct_proto_id);

	if (direct_matrix_entry_ptr != OPC_NIL)
		{
		/* Check if the given process is in the list of target protocols.	*/
		redistributes_direct = (ip_cmn_rte_table_redist_info_search (direct_matrix_entry_ptr, proc_id, addr_family) != OPC_NIL);
		}

	FRET (redistributes_direct);
	}
