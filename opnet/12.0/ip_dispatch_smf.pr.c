...
const char ip_dispatch_smf_pr_c [] = "MIL_3_Tfile_Hdr_ 120A 30A op_runsim 7 464DAC7D 464DAC7D 1 wn12jh Jim@Hauser 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 10de 3                                                                                                                                                                                                                                                                                                                                                                                                      ";
#include <string.h>
...
#include "ip_dynamic_assignments.h"
...
#define DYN_ASSIGNMENT		((intrpt_type == OPC_INTRPT_REMOTE) && \
							 (intrpt_code == IPC_DYNAMIC_ASSIGNMENTS_INTRPT_CODE))
...
#define FAIL_REC            ((invoke_mode == OPC_PROINV_DIRECT) &&  \
							 (intrpt_code < IPC_OBSERVER_INTRPT_LIMIT) && \
							 (intrpt_code > IPC_OBSERVER_INTRPT_LOWER_LIMIT))
...
/* Do not consider IPv6 configuration is user does not want us to.	*/
/* Integer instead of Boolean to capture unread status.				*/
static int	consider_ipv6_config = -1;
#define IPV6_CONFIG_STATUS_NOT_READ	 -1
...
#define IPC_INTF_OPERATIONAL_STATUS_DOWN	0
...
static Boolean 				ip_rte_manet_proto_compare (int incoming_pk_proto, IpT_Rte_Protocol intf_manet_proto);
...
static Boolean					ip_dispatch_intf_is_active (Objid intf_objid);
...
static void					ip_dispatch_icmp_pk_higher_layer_forward (Packet* ip_pkptr, IpT_Dgram_Fields* pkt_fields_ptr, Ici* intf_ici_ptr,
...
/********************************************************************************/
/* BEGIN CODE FOR END-TO-END DELAY AND JITTER STATISTICS						*/
/********************************************************************************/


#define		IPC_ETE_STAT_DELAY_FLAG			(0x01 << 0)
#define 	IPC_ETE_STAT_JITTER_FLAG		(0x01 << 1)
#define		IPC_ETE_STAT_NO_RECORDS_FLAG	(0x01 << 2)

typedef struct IpT_Ete_Stat_Data
	{
	List					table;
	OpT_uInt16				next_stat_index;
	OpT_uInt8				flags;
	} IpT_Ete_Stat_Data;

typedef struct IpT_Ete_Stat_Record
	{
	Stathandle					delay_handle;
	Stathandle					jitter_handle;
	OmsT_Stat_Data				stat_data;
	InetT_Address				src_addr;
	InetT_Address				dest_addr;
	} IpT_Ete_Stat_Record;

static int			ete_stat_max_dim = -1;

#define ip_dispatch_ete_stats_requested() 			(ete_stat_data_ptr != OPC_NIL)
#define	ip_dispatch_ete_stat_delay_request() 		(ete_stat_data_ptr->flags |= IPC_ETE_STAT_DELAY_FLAG)
#define	ip_dispatch_ete_stat_jitter_request() 		(ete_stat_data_ptr->flags |= IPC_ETE_STAT_JITTER_FLAG)
#define ip_dispatch_ete_stat_delay_is_requested()	(ete_stat_data_ptr->flags & IPC_ETE_STAT_DELAY_FLAG)
#define ip_dispatch_ete_stat_jitter_is_requested()	(ete_stat_data_ptr->flags & IPC_ETE_STAT_JITTER_FLAG)
#define ip_dispatch_ete_stat_set_no_records_flag()		(ete_stat_data_ptr->flags |= IPC_ETE_STAT_NO_RECORDS_FLAG)
#define	ip_dispatch_ete_stat_no_records_flag_is_set()	(ete_stat_data_ptr->flags & IPC_ETE_STAT_NO_RECORDS_FLAG)
#define ip_dispatch_ete_stat_reset_no_records_flag()	(ete_stat_data_ptr->flags &= ~IPC_ETE_STAT_NO_RECORDS_FLAG)  

static void			ip_dispatch_ete_stats_init (void);
static void			ip_dispatch_ete_stats_write (IpT_Dgram_Fields* pk_fd_ptr, Packet* pkptr);
static void			ip_dispatch_ete_stat_record_init (IpT_Ete_Stat_Record* record_ptr, IpT_Dgram_Fields* pk_fd_ptr, Boolean register_handles);	

/********************************************************************************/
/* END CODE FOR END-TO-END DELAY AND JITTER STATISTICS							*/
/********************************************************************************/


/*	ip_dynamic_assignments modifications */

static void 				ip_dispatch_dynamic_assignment_handle ();
static Compcode				ip_dispatch_dynamic_address_create (IpT_Dynamic_Assignment *assignment_ptr);
static Compcode				ip_dispatch_dynamic_prefix_create (IpT_Dynamic_Assignment *assignment_ptr);
static Compcode				ip_dispatch_dynamic_address_remove (IpT_Dynamic_Assignment *assignment_ptr);
static Compcode				ip_dispatch_dynamic_default_route_handle (IpT_Dynamic_Assignment *assignment_ptr, Boolean insert);
...
typedef struct
	...
	Prohandle	              		observer_prohandle                              ;	/* Handle to the ip_observer process */
	List*	                  		dynamic_addr_nato_key_lptr                      ;	/* List for Nato table entries of dynamic addresses */
	IpT_Ete_Stat_Data*	     		ete_stat_data_ptr                               ;	/* Data structure to record per conversation end-to-end delay and delay variation. */
	} ip_dispatch_smf_state;

#define module_data             		op_sv_ptr->module_data
#define is_ip_initialized       		op_sv_ptr->is_ip_initialized
#define child_ptr               		op_sv_ptr->child_ptr
#define default_ttl             		op_sv_ptr->default_ttl
#define radio_intf_list_ptr     		op_sv_ptr->radio_intf_list_ptr
#define slot_table_lptr         		op_sv_ptr->slot_table_lptr
#define ip_info_ptr             		op_sv_ptr->ip_info_ptr
#define dynamic_routing_enabled 		op_sv_ptr->dynamic_routing_enabled
#define link_iface_table_ptr    		op_sv_ptr->link_iface_table_ptr
#define own_process_record_handle		op_sv_ptr->own_process_record_handle
#define proc_model_name         		op_sv_ptr->proc_model_name
#define comp_attr_objid         		op_sv_ptr->comp_attr_objid
#define subnet_objid            		op_sv_ptr->subnet_objid
#define interface_table_size    		op_sv_ptr->interface_table_size
#define slot_iface_map_array    		op_sv_ptr->slot_iface_map_array
#define iface_addressing_mode   		op_sv_ptr->iface_addressing_mode
#define oms_basetraf_process_id 		op_sv_ptr->oms_basetraf_process_id
#define tcpip_header_comp_info_ptr		op_sv_ptr->tcpip_header_comp_info_ptr
#define per_interface_comp_info_ptr		op_sv_ptr->per_interface_comp_info_ptr
#define per_virtual_circuit_comp_info_ptr		op_sv_ptr->per_virtual_circuit_comp_info_ptr
#define igmp_host_process_handle		op_sv_ptr->igmp_host_process_handle
#define pim_sm_process_handle   		op_sv_ptr->pim_sm_process_handle
#define custom_mrp_process_handle		op_sv_ptr->custom_mrp_process_handle
#define routing_prohandle       		op_sv_ptr->routing_prohandle
#define invoke_prohandle        		op_sv_ptr->invoke_prohandle
#define mcast_rte_protocol      		op_sv_ptr->mcast_rte_protocol
#define passive_rip             		op_sv_ptr->passive_rip
#define crt_export_time_lptr    		op_sv_ptr->crt_export_time_lptr
#define global_crt_export_time_lptr		op_sv_ptr->global_crt_export_time_lptr
#define unknown_instrm_index_lptr		op_sv_ptr->unknown_instrm_index_lptr
#define static_rte_info         		op_sv_ptr->static_rte_info
#define ad_hoc_routing_protocol_str		op_sv_ptr->ad_hoc_routing_protocol_str
#define vrf_export_time_lptr    		op_sv_ptr->vrf_export_time_lptr
#define observer_prohandle      		op_sv_ptr->observer_prohandle
#define dynamic_addr_nato_key_lptr		op_sv_ptr->dynamic_addr_nato_key_lptr
#define ete_stat_data_ptr       		op_sv_ptr->ete_stat_data_ptr
...
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((ip_dispatch_smf_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
ip_dispatch_do_init (void)
	...
	module_data.icmp_process_handle = invalid_prohandle;
	module_data.ipv6_prohandle = invalid_prohandle;
	module_data.ipv6_ra_prohandle = invalid_prohandle;
	...
	/* Check whether this node is a HAIPE device.  If so, then the	*/
	/* flag needs to be checked.									*/
	if (haipe_support_is_device_haipe_gateway (module_data.node_id))
		module_data.flags |= IPC_NODE_FLAG_HAIPE;
	...
	ip_dispatch_ete_stats_init ();	

	/* Read global attribute that indicates whether IPv6 configuration must be read.	*/
	if (consider_ipv6_config == IPV6_CONFIG_STATUS_NOT_READ)
		op_ima_sim_attr_get (OPC_IMA_INTEGER, "IPv6 Configuration", &consider_ipv6_config);
...
ip_dispatch_init_phase_2 (void)
	...
	int						num_phy_available_interfaces = 0;
	int						num_mgtwy_interfaces = 0;		
	int						active_manet_protocols = 0;
	...
	if (module_data.gateway_status == OPC_TRUE)
		...
		/* attributes.												*/
		/* IPv6 configuration must be read only if the global		*/
		/* attribute to IPv6 configuration is turned ON.			*/
		if (consider_ipv6_config)
			{
	...
	for (i = 0; i < module_data.num_interfaces; i++)
		...
		/* this interface also.											*/
		if (consider_ipv6_config && module_data.gateway_status == OPC_TRUE)
			{
		...
		if ((line_objid != OPC_OBJID_INVALID) || (IpC_Intf_Status_Loopback == intf_status) ||
			...
			/* Read in the Compression Information, Access Groups,		*/
			...
			if ((intf_status == IpC_Intf_Status_Active) || (intf_status == IpC_Intf_Status_Tunnel) ||
				...
				if (ip_interface_routing_protocols_contains (routing_protocols_lptr, IpC_Rte_Ospf) ||
					ip_interface_routing_protocols_contains (routing_protocols_lptr, IpC_Rte_Ospf3) ||
			...
			if (ip_interface_routing_protocols_contains (routing_protocols_lptr, IpC_Rte_Dsr))
				...
				iface_info_ptr->flags |= IPC_INTF_FLAG_MANET_ENABLED;
				active_manet_protocols |= IPC_RTE_PROTO_OLSR;
				}
			...
			if (!ip_rte_intf_manet_enabled (iface_info_ptr)  || ip_manet_proto_type_is_aodv(&module_data))
	...
	if (module_data.gateway_status == OPC_TRUE)
		...
		if (consider_ipv6_config && intf_objid_lookup_tables.ipv6_table != IpC_Intf_Name_Objid_Table_Invalid)
			{
	...
	for (i = 0, intf_list_size = op_prg_list_size (module_data.interface_table_ptr); i < intf_list_size; i++)
		...
		if (iface_info_ptr->phys_intf_info_ptr->intf_status == IpC_Intf_Status_Tunnel)
			...
			/* Count the number of available active interfaces.				*/
			if (iface_info_ptr->phys_intf_info_ptr->intf_status == IpC_Intf_Status_Active && 
				iface_info_ptr->phys_intf_info_ptr->link_status == 1)
				num_phy_available_interfaces++;
			}
		}
	...
	/* If this is a gateway and MANET is enabled, check if this		*/
	/* is a MANET Gateway.											*/
	if ((module_data.gateway_status == OPC_TRUE || passive_rip == OPC_TRUE) &&
		(ip_manet_is_enabled (&module_data) == OPC_TRUE))
		{
		int manet_gateway_status_toggle_int;
	
		/* If more than one MANET protocol is defined in this node	*/
		/* write a log message indicating an unsupported 			*/
		/* configuration.											*/
		if (! (active_manet_protocols == IPC_RTE_PROTO_DSR || active_manet_protocols == IPC_RTE_PROTO_TORA 
			|| active_manet_protocols == IPC_RTE_PROTO_AODV || active_manet_protocols == IPC_RTE_PROTO_OLSR))
			{
			/* There are more than one MANET protocol. Model 		*/
			/* supports only one, the MANET protocol runnning is...	*/
			ipnl_manet_multiple_manet_protocols_error (module_data.interface_table_ptr, ip_manet_proto_type(&module_data));
			}
	
		/* This router may be a MANET Gateway, check its MANET	*/
		/* configuration.										*/
		/* Check if any MANET Routing protocols have been enabled	*/
		op_ima_obj_attr_get_toggle (module_data.module_id, "manet_mgr.MANET Gateway", &manet_gateway_status_toggle_int);
		module_data.manet_info_ptr->manet_gateway_status = manet_gateway_status_toggle_int == 1 ? OPC_TRUE :  OPC_FALSE;

		/* If the MANET gateway is enabled in this node check	*/
		/* for misconfigurations that force to disable this		*/
		/* feature (MANET Gateway).								*/
		if (ip_manet_gateway_enabled(&module_data))
			{
			/* MANET Gateway capability is available only for	*/
			/* AODV and OLSR (with static routes only)			*/
			if (! ((ip_manet_proto_type_is_aodv (&module_data)) || (ip_manet_proto_type_is_olsr (&module_data))))
				{		
				/* Currently, AODV is the only MANET protocol that supports 	*/
				/* MANET gateway functionality.	(Also OLSR)						*/
				
				/* Disable the MANET gateway capability.		*/
				module_data.manet_info_ptr->manet_gateway_status = OPC_FALSE;
				 
				/* Write a simulation log message.				*/
				ipnl_manet_invalid_manet_gateway_protocol_error (ip_manet_proto_type(&module_data));
				
				}
			/* A router with only one interface cannot be a MANET gateway.	*/
			else if (num_phy_available_interfaces == 1)
				{
				/* Write a simulation log message.				*/
				ipnl_manet_gateway_with_only_one_interface_error (module_data.interface_table_ptr);

				/* Disable the MANET gateway capability.			*/				
				module_data.manet_info_ptr->manet_gateway_status = OPC_FALSE;
				}
				/* If AODV runs on all interfaces then this node cannot be a MANET gateway.	*/
			else if (num_mgtwy_interfaces == num_phy_available_interfaces)
				{
				/* Write a simulation log message.				*/
				ipnl_manet_gateway_manet_active_on_all_interfaces_error ();
	...
	if ((module_data.gateway_status == OPC_TRUE || passive_rip == OPC_TRUE) &&
		((ip_manet_is_enabled(&module_data) == OPC_FALSE) || ip_manet_gateway_enabled(&module_data)))
		{
	...
	observer_prohandle = op_pro_create ("ip_observer", OPC_NIL);
	op_pro_invoke (observer_prohandle, OPC_NIL);
	
	/* We have to migrate IP immediately as these interrupts are being steered */
	op_intrpt_type_register (OPC_INTRPT_FAIL, observer_prohandle);
	op_intrpt_type_register (OPC_INTRPT_RECOVER, observer_prohandle);
	...
	/* Only need to read the HAIPE attributes if this is a HAIPE device. */
	if (ip_node_is_haipe (&module_data))
		haipe_support_attributes_read (&module_data);	
...
ip_dispatch_subintf_info_read (IpT_Interface_Info* parent_intf_ptr, Objid subintf_info_attr_objid, 
	...
	for (count_i = 0; count_i <  num_subinterfaces; count_i++)
		...
		status = ip_dispatch_intf_is_active (ith_subintf_objid);
...
ip_dispatch_host_ipv6_attrs_read (Objid iface_description_objid, IpT_Interface_Info* iface_info_ptr)
	...
	/* If the global IPv6 status attribute is disabled, then skip 	*/
	/* operation. The status attribute enables users to turn off	*/
	/* IPv6 without scrubbing IPv6 configuration in the network.	*/
	if (!consider_ipv6_config)
		FOUT;
...
ip_dispatch_routing_options_add (List* routing_protocols_lptr)
	...
	for (i_th_protocol = 0; i_th_protocol < num_rte_protocols; i_th_protocol++)
		...
		switch (i_th_routing_protocol_id)
			...
			/* This checks are added to support MANET gateway capabilities.	*/
			/* Currently MANET protocols are not processed, it is just 		*/
			/* needed that they are recognized as valid routing protocols.	*/
			case IpC_Rte_Aodv:
			case IpC_Rte_Dsr:
			case IpC_Rte_Olsr:
			case IpC_Rte_Tora:
...
ip_dispatch_distribute_routing_info (void)
	...
	/* An exception is made for MANET hosts							*/
	if (((module_data.gateway_status == OPC_TRUE) || (ip_manet_is_enabled (&module_data))) &&
		(routing_table_import_export_flag != IP_RTE_TABLE_IMPORT))
		{		
		...
		if (ip_rte_node_ipv6_active (&module_data))
			{
			/* IPv6 routing table comes from different attributes on gateways and MANET hosts		*/
			if (module_data.gateway_status == OPC_TRUE)
				{
				...
				{
				op_ima_obj_attr_get (module_data.ip_parameters_objid, "V6 Static Routing Table", &static_rte_table_objid);
				ip_rte_table_parse (static_rte_table_objid, module_data.ip_route_table, &module_data, InetC_Addr_Family_v6);
				}
			}
	...
	if ((module_data.gateway_status) && (ip_rte_node_ipv4_active (&module_data)))
		{
...
ip_dispatch_sv_init (void)
	...
	Objid		sys_info_objid, sys_info_row_objid;
	char		os_str [64];	
	int			num_modules, module_index;
	...
	module_data.seg_buf_ptr = op_sar_buf_create (OPC_SAR_BUF_TYPE_SEGMENT, OPC_SAR_BUF_OPT_DEFAULT);
	module_data.reseg_buf_ptr = op_sar_buf_create (OPC_SAR_BUF_TYPE_RESEGMENT, OPC_SAR_BUF_OPT_DEFAULT);
	module_data.rsm_buf_ptr = op_sar_buf_create (OPC_SAR_BUF_TYPE_REASSEMBLY, OPC_SAR_BUF_OPT_DEFAULT);

	/* Reassembly buffer must be flushed if segments of a packet do not arrive within reasonable time.	*/
	op_sar_buf_options_set (module_data.rsm_buf_ptr, OPC_SAR_BUF_OPT_FLUSH_TIMED, IPC_REASSEMBLY_TIMEOUT);

	/* If this node is running IOS-XR, policies will be in RPL.	*/
	if (op_ima_obj_attr_exists (module_data.node_id, "System Information"))
		{
		op_ima_obj_attr_get_objid (module_data.node_id, "System Information", &sys_info_objid);
		num_modules = op_topo_child_count (sys_info_objid, OPC_OBJTYPE_GENERIC);

		for (module_index = 0; module_index < num_modules; module_index++)
			{	
			sys_info_row_objid = op_topo_child (sys_info_objid, OPC_OBJTYPE_GENERIC, module_index);
			op_ima_obj_attr_get_str (sys_info_row_objid, "OS Type", 64, os_str);
			if (strcmp (os_str, "IOS XR") == 0)
				{
				module_data.flags |= IPC_NODE_FLAG_IOS_XR;
				break;
				}
			}
		}	
...
ip_register_routerid_as_local_netaddr ()
	...
	for (index = 0; index < intf_table_size ; ++index)
		...
		if ((ip_rte_intf_unnumbered (iface_info_ptr)) &&
			...
			(ip_interface_routing_protocols_contains (iface_info_ptr->routing_protocols_lptr, IpC_Rte_Ospf3) == OPC_TRUE)))
...
ip_interface_routing_protocols_obtain (Objid intf_info_objid, Objid ipv6_attrs_objid,
	...
	static char				sim_attr_rte_prot_str [32];
	...
	if (!initialization_complete)
        ...
        if (strcmp (ip_dyn_rte_spec, "Default") != 0)
			...
			/* Copy ip_dyn_rte_spec in the static string	*/
			strcpy (sim_attr_rte_prot_str, ip_dyn_rte_spec);
	...
	if (!module_data.gateway_status)
		...
		if ((sim_attr_rte_prot_lptr != OPC_NIL))
			...
			/* The above return is incorrect and causes no IPv6 information to be parsed.	*/
			/* Simply copy the contents of static string in ip_dyn_rte_str and keep going	*/
			strcpy (ip_dyn_rte_spec, sim_attr_rte_prot_str);
			}
		
		/* End nodes can have MANET routing protocols running. 		*/
		else if (!strcmp (ad_hoc_routing_protocol_str, "DSR"))
			{
		...
		/* there.													*/
			...
			/* If the global simulation attribute is a MANET protocol, router should exit	*/
			/* This is because for a MANET, no assumption about IPv4 or IPv6 can be made	*/
			/* before parsing its local addressing mode, and it is possible that the router	*/
			/* may be badly configured with a local MANET and a global MANET.				*/
			if ((!strcmp (ip_dyn_rte_spec, "OLSR")) || (!strcmp (ip_dyn_rte_spec, "AODV")) || 
				(!strcmp (ip_dyn_rte_spec, "DSR")) || (!strcmp (ip_dyn_rte_spec, "TORA")))
				FRET (sim_attr_rte_prot_lptr);
		...
		if (OPC_OBJID_INVALID != ipv6_attrs_objid)
			{
			/* Read IPv6 routing protocols only if link-local address is enabled on the interface.	*/
			/* Otherwise, IPv6 will not be functional on the interface.								*/
			op_ima_obj_attr_get_str (ipv6_attrs_objid, "Link-Local Address", IPC_MAX_STR_SIZE, addr_str);

			if (!(strcmp (addr_str, "Not Active") == 0))
				{
...
ip_rte_proto_string_parse (char* routing_proto_str, List* active_custom_rte_proto_label_lptr)
	...
	for (ith_protocol = 0; ith_protocol < num_rte_protocols; ith_protocol++)
		...
		if (strcmp (ith_protocol_name, "RIP") == 0)
		...
		else if (strcmp (ith_protocol_name, "IS-IS (IPv6)") == 0)
			{
			/* M-ISIS protocol has been specified to be used on this	*/
			/* interface. This is for v6 interface only					*/
			/* and doesn't really introduce a new routing protocol. It  */
			/* is mainly used for initialization part of the isis process */
			routing_protocol = IpC_Rte_Misis;
			}
			...
			/* interface.											*/
			routing_protocol = IpC_Rte_Ospf3;
			}
...
static Boolean 
ip_rte_manet_proto_compare (int incoming_pk_proto, IpT_Rte_Protocol intf_manet_proto)
	{
	/* Compare the protocol of the incoming ip_dgram packet with interface protocol		*/
	/* Since they are in different enums, a separate function is written				*/
	FIN (ip_rte_manet_proto_compare (...));
	
	if ((incoming_pk_proto == IpC_Protocol_Aodv) && (intf_manet_proto == IpC_Rte_Aodv))
		{
		FRET (OPC_TRUE);
		}
	else if ((incoming_pk_proto == IpC_Protocol_Dsr) && (intf_manet_proto == IpC_Rte_Dsr))
		{
		FRET (OPC_TRUE);
		}
	else if ((incoming_pk_proto == IpC_Protocol_Tora) && (intf_manet_proto == IpC_Rte_Tora))
		{
		FRET (OPC_TRUE);
		}
	else
...
ip_rte_datagram_higher_layer_forward (Packet *frag_pk_ptr)
	...
	if ((ip_mcast_data_pkt_on_rte == OPC_TRUE) || 
		((ip_pkptr = ip_frag_sup_insert (module_data.rsm_buf_ptr, frag_pk_ptr)) != OPC_NIL))
		{
		...
		if (ip_proto_is_icmp_v4_or_v6 (pkt_fields_ptr->protocol))
			...
			/* ICMP packet type.											*/
			ip_dispatch_icmp_pk_higher_layer_forward (ip_pkptr, pkt_fields_ptr, intf_ici_ptr, intf_ici_fdstruct_ptr);
			}
		
		/* Destroy this packet if a) either MANET is not enabled or b) different	*/
		/* MANET protocol is running on the interface								*/
		else if (((pkt_fields_ptr->protocol == IpC_Protocol_Dsr) || 
			(pkt_fields_ptr->protocol == IpC_Protocol_Aodv) || 
			(pkt_fields_ptr->protocol == IpC_Protocol_Tora)) && 
			((!ip_manet_is_enabled (&module_data)) || 
				(!ip_rte_manet_proto_compare (pkt_fields_ptr->protocol, module_data.manet_info_ptr->rte_protocol))))
			{
			switch (pkt_fields_ptr->protocol)
				{
			...
			ip_rte_dgram_discard (&module_data, frag_pk_ptr, intf_ici_ptr, "MANET packet received when appropriate MANET protocol is not running on this node");
			...
			{			
		/* End-to-end delay stats must be written for explicit demand packets.	*/
			/* Stats are not written for broadcast traffic since they are usually	*/
			/* control plane packets and we are not interested in their delays.		*/
			if (ip_dispatch_ete_stats_requested () && !intf_ici_fdstruct_ptr->broadcast)
				ip_dispatch_ete_stats_write (pkt_fields_ptr, ip_pkptr);
			...
			{
			/* End-to-end delay stats will be written only for packets going up.	*/
			/* For tunnel packets, separate ETE delay stats exist.					*/
			/* Stats are not written for broadcast traffic since they are usually	*/
			/* control plane packets and we are not interested in their delays.		*/
			if (ip_dispatch_ete_stats_requested () && !intf_ici_fdstruct_ptr->broadcast)
				ip_dispatch_ete_stats_write (pkt_fields_ptr, ip_pkptr);
...
ip_directly_connected_networks_for_interface_handle (IpT_Interface_Info* interface_ptr, int intf_index, Boolean insert)
	...
	if ((ip_rte_intf_ipv4_active (interface_ptr)) &&
		...
		for (count_j = -1; count_j < num_secondary_addrs; count_j++)
			...
			if (!ip_rte_intf_is_msfc_alt (interface_ptr))
				...
				if (insert)
					{
					/* If this interface is running any MANET protocol, insert a /32 route		*/
					/* This is done because all MANET prefixes are propogated as /32			*/
					/* no matter what interface configuration is. Thus any MANET interface 		*/
					/* has no notion of a "network" and rather should only have a /32 prefix	*/
					if (ip_rte_intf_manet_enabled (interface_ptr))
						dest_prefix = ip_cmn_rte_table_dest_prefix_create (intf_addr, inet_smask_from_length_create (IPC_V4_ADDR_LEN));
					...
					{
					/* If this interface runs MANET, a /32 mask was inserted. Look for that for removal		*/
					if (ip_rte_intf_manet_enabled (interface_ptr))
						dest_prefix = ip_cmn_rte_table_dest_prefix_create (intf_addr, inet_smask_from_length_create (IPC_V4_ADDR_LEN));
	...
	if (ip_rte_intf_ipv6_active (interface_ptr))
		...
		for (count_j = 0; count_j < num_ipv6_global_addrs; count_j++)
			...
			if (insert)
				{
				/* If this interface is running MANET, a /128 mask is inserted for directly connected networks	*/
				/* Comments are given in the case above, where /32 routes were inserted for IPv4.				*/
				if (ip_rte_intf_manet_enabled (interface_ptr))
					dest_prefix = inet_address_range_create (ip_rte_intf_ith_gbl_ipv6_addr_get_fast (interface_ptr, count_j), 
						inet_smask_from_length_create (IPC_V6_ADDR_LEN));
				...
				{
				/* If this interface is running MANET, /128 route was added. Look for the same for removal. 	*/
				if (ip_rte_intf_manet_enabled (interface_ptr))
					dest_prefix = inet_address_range_create (ip_rte_intf_ith_gbl_ipv6_addr_get_fast (interface_ptr, count_j), 
						inet_smask_from_length_create (IPC_V6_ADDR_LEN));
...
ip_dispatch_route_table_init (void)
	...
	module_data.ip_route_table->convg_handle = oms_routing_convergence_register (OmsC_IP_Forwarding_Table, 
		module_data.node_id, OPC_NIL, convergence_stat_names, parent_convergence_stat_names, OPC_STAT_INDEX_NONE);
	...
	/* Static routing is supported for MANET hosts too			*/
	if ((module_data.gateway_status == OPC_TRUE) || (ip_manet_is_enabled (&module_data)))
...
ip_dispatch_fail_rec_handle (int intrpt_code)
	...
	observer_client_info_ptr = (IpT_Observer_Client_Info*) op_intrpt_state_ptr_get ();
...
ip_dispatch_intf_info_objid_get (int intf_index, const IpT_Intf_Info_Attrs* intf_info_attrs_ptr,
	...
	if (intf_index < intf_info_attrs_ptr->num_aggr_interfaces)
		...
		*intf_status_ptr = IpC_Intf_Status_Group;
		...
		intf_active = ip_dispatch_intf_is_active (intf_info_objid);
...
ip_dispatch_bgutil_packet_process (Packet* pk_ptr, Ici* intf_ici_ptr, InetT_Address* dest_addr_ptr)
	...
	/* If this is a security demand, then log the life cycle in an	*/
	/* OT table. Do this before calling the route-record function	*/
	/* because the route-record function will destroy the route 	*/
	/* information.													*/
	...
	/* If this packet is carrying route information, then that must	*/
	/* be written to the gdf file.									*/
	if (op_pk_encap_flag_is_set (pk_ptr, OMSC_RR_ENCAP_FLAG_INDEX))
		{
		/* For multicast demands, we need to pass the actual destination	*/
		/* name since it was not set at the time of sending the packet.		*/
		if (inet_address_is_multicast (*dest_addr_ptr) == OPC_TRUE)
			...
			dest_node_name_ptr = dest_node_name;
			}
		else
			dest_node_name_ptr = OPC_NIL;

		oms_rr_info_record_in_file (pk_ptr, dest_node_name_ptr);
		}
	...
	if (op_pk_encap_flag_is_set (pk_ptr, OMSC_BGUTIL_ENCAP_FLAG_INDEX) == OPC_TRUE)
		...
		op_pk_encap_pk_access (pk_ptr, (char *)"bgutil_tracer", &bgutil_pkptr);
	...
	if (op_pk_encap_flag_is_set (pk_ptr, OMSC_DEMAND_ENCAP_FLAG_INDEX) == OPC_TRUE)
		...
		op_pk_encap_pk_access (pk_ptr, (char *)"demand_info", &demand_pkptr);
...
ip_manet_rte_mgr_init (void)
	...
	if (record_handle_list_size == 0)
		...
		op_sim_error (OPC_SIM_ERROR_WARNING, "Unable to find manet_rte_mgr process in the node.",
			"OLSR requires the presence of this process.");
...
ip_dispatch_ipv6_ra_process_create (void)
	...
	if (! ip_rte_node_is_gateway (&module_data))
		...
		if ((ip_rte_intf_is_smart (ip_intf_ptr)) &&
			...
			{
			/* This can happen if the interface has a v6 address,	*/
			/* but is not connected.								*/
#ifndef OPD_NO_DEBUG 
			if (op_prg_odb_ltrace_active ("ip"))
				{
				/* The lower layer has not initialized yet.		*/
				op_prg_odb_print_major ("Error in ip_dispatch_ipv6_ra_process_create",
					"The ARP layer has not completed initialization", 
					"Interface may be unconnected", 
					"RA process will not be created", OPC_NIL);
				}
#endif
			FOUT;
			}
		...
		switch (icmp_pk_fields_ptr->message_type)
			...
			case IpC_Icmp_Message_Type_Rtr_Solicit:
				...
				/* process.									*/
					...
					{
					if (op_pro_valid (module_data.ipv6_ra_prohandle))
						op_pro_invoke (module_data.ipv6_ra_prohandle, ip_pkptr);
					else
						{
						/* RA disabled on this node. No child process. Silently discard.	*/
						op_pk_destroy (ip_pkptr);
						op_ici_destroy (intf_ici_ptr);
						ip_rte_ind_ici_fdstruct_destroy (intf_ici_fdstruct_ptr);
						}
...
static void
ip_dispatch_ete_stats_init (void)
	{
	Boolean					delay_requested, jitter_requested;
	IpT_Ete_Stat_Record*	record_ptr;
	Stathandle				delay_hndl, jitter_hndl;
	
	/** Initializes data structures if ETE delay statistics have been requested by user.	**/
	FIN (ip_dispatch_ete_stats_init ());

	/* Statistics will be collected per conversation, so this is a dimensioned stat.		*/
	/* Get the maximum dimension in a global variable. Users will not need to change code	*/
	/* if they change the statistic dimension. Do this only once at start of sim.			*/
	if (-1 == ete_stat_max_dim)
		op_stat_dim_size_get ("IP.End-to-end Delay (sec)", OPC_STAT_LOCAL, &ete_stat_max_dim);

	/* We assume that the delay and jitter stats will have the same dimension. If user	*/
	/* configures dimension of jitter to be less, some recoverable/diagnostics may 		*/
	/* occur when we reach the limit.													*/

	/* Check if this stat has been requested for this node. The only way to do that is 	*/
	/* to register the statistic and check the validity of the returned stathandle.		*/
	/* The statistic table will be created if either of the two (delay, jitter) are		*/
	/* being collected for this node.													*/
	delay_hndl = op_stat_reg ("IP.End-to-end Delay (sec)", 0, OPC_STAT_LOCAL);
	delay_requested = op_stat_valid (delay_hndl);
	jitter_hndl = op_stat_reg ("IP.End-to-end Delay Variation (sec)", 0, OPC_STAT_LOCAL);
	jitter_requested = op_stat_valid (jitter_hndl);

	if (!(delay_requested || jitter_requested))
		FOUT;
	
	/* User wants delay and/or jitter statistics.	*/
	ete_stat_data_ptr = (IpT_Ete_Stat_Data *) op_prg_mem_alloc (sizeof (IpT_Ete_Stat_Data));
	op_prg_list_init (&ete_stat_data_ptr->table);

	/* Store the registered handles for delay and jitter. These correspond to dimension	*/
	/* zero. If we do not use these stathandles, then the View Results display will 	*/
	/* contain one unused statistic that is grayed out.									*/
	record_ptr = (IpT_Ete_Stat_Record *) op_prg_mem_alloc (sizeof (IpT_Ete_Stat_Record));
	record_ptr->delay_handle = 	delay_hndl;
	record_ptr->jitter_handle = jitter_hndl;
	op_prg_list_insert (&ete_stat_data_ptr->table, record_ptr, OPC_LISTPOS_HEAD);

	/* Set a flag indicating that this record is not assigned to any flow and can be used	*/
	/* later by incoming traffic.															*/
	ip_dispatch_ete_stat_set_no_records_flag ();
	
	if (delay_requested)
		ip_dispatch_ete_stat_delay_request ();

	if (jitter_requested)
		ip_dispatch_ete_stat_jitter_request ();
	
	/* We have used up one statistic to check if this stat has been selected. We can try and 	*/
	/* save the handle for future use but that will require some extra complexity. So, we will 	*/
	/* just start from the next handle. So, if the dimension is 100, we will be able to record	*/
	/* only 99 conversations. We can probably live with that.									*/
	ete_stat_data_ptr->next_stat_index = 1;

	FOUT;
	}

static void
ip_dispatch_ete_stat_record_init (IpT_Ete_Stat_Record* record_ptr, IpT_Dgram_Fields* pk_fd_ptr, Boolean register_handles)
	{
	char		flow_key [2*INETC_ADDR_STR_LEN + 10], src_str [INETC_ADDR_STR_LEN], dest_str [INETC_ADDR_STR_LEN];

	/** Initialize various fields inside the end-to-end statistics record.	This function 	**/
	/** is called from different places. Sometimes, registration of stathandles is require,	**/
	/** sometimes not. See comments inside calling functions for details.					**/
	FIN (ip_dispatch_ete_stat_record_init ());

	record_ptr->src_addr = inet_address_copy (pk_fd_ptr->src_addr);
	record_ptr->dest_addr = inet_address_copy (pk_fd_ptr->dest_addr);

	/* Register the stat handles and annotate them with the flow addresses.	*/
	inet_address_print (src_str, pk_fd_ptr->src_addr);
	inet_address_print (dest_str, pk_fd_ptr->dest_addr);
	sprintf (flow_key, "%s --> %s", src_str, dest_str);

	if (ip_dispatch_ete_stat_delay_is_requested ())
		{
		if (register_handles)
			record_ptr->delay_handle = op_stat_reg ("IP.End-to-end Delay (sec)", 
				ete_stat_data_ptr->next_stat_index, OPC_STAT_LOCAL);

		op_stat_annotate (record_ptr->delay_handle, flow_key);
		}

	if (ip_dispatch_ete_stat_jitter_is_requested ())
		{
		if (register_handles)
			record_ptr->jitter_handle = op_stat_reg ("IP.End-to-end Delay Variation (sec)", 
				ete_stat_data_ptr->next_stat_index, OPC_STAT_LOCAL);
		
		op_stat_annotate (record_ptr->jitter_handle, flow_key);

		/* Additional state initialization if jitter data is to be collected.	*/
		oms_stat_data_init (&record_ptr->stat_data);
		}
	FOUT;
	}
...
static void
ip_dispatch_ete_stats_write (IpT_Dgram_Fields* pk_fd_ptr, Packet* pkptr)
	{
	int						num_records, record_index;
	IpT_Ete_Stat_Record*	record_ptr;
	double					cur_delay;

	/** Write end-to-end delay and jitter statistics for packets being sunk by this node.	**/
	FIN (ip_dispatch_ete_stats_write ());
	
	/* Statistics will be written only for explicit traffic.	*/
	if (op_pk_encap_flag_is_set (pkptr, OMSC_BGUTIL_ENCAP_FLAG_INDEX))
		FOUT;

	/* Statistics are written only for unicast packets.	Broadcast check is	*/
	/* made by the calling function since that info is available there. We	*/
	/* make a multicast check here.											*/
	if (inet_address_is_multicast (pk_fd_ptr->dest_addr))
		FOUT;

	/* During the initialization phase, we registered stats for dimension 	*/
	/* zero in order to check if the stat was requested by the user. If 	*/
	/* this is the first flow for which the stat is being recorded, then we	*/
	/* can use those stathandles. If we do not do this and just move to the	*/
	/* next one, then we are going to have one empty stat when we view the	*/
	/* results.																*/
	if (ip_dispatch_ete_stat_no_records_flag_is_set ())
		{
		/* The only record present in the list is available for use.	*/
		record_ptr = (IpT_Ete_Stat_Record *) op_prg_list_access (&ete_stat_data_ptr->table, OPC_LISTPOS_HEAD);

		/* The various fields in the record must be set. But the handles	*/
		/* for the statistics have already been registered.					*/
		ip_dispatch_ete_stat_record_init (record_ptr, pk_fd_ptr, OPC_FALSE /* no need to register handles */);

		/* For subsequent flows, new stat handles must be registered.	*/
		ip_dispatch_ete_stat_reset_no_records_flag ();
		}
	else
		{
		/* Go through the table and see if we have a stat element for this flow.	*/
		num_records = op_prg_list_size (&ete_stat_data_ptr->table);

		for (record_index = 0; record_index < num_records; record_index++)
			{
			record_ptr = (IpT_Ete_Stat_Record *) op_prg_list_access (&ete_stat_data_ptr->table, record_index);

			if (inet_address_equal (record_ptr->src_addr, pk_fd_ptr->src_addr) &&
				inet_address_equal (record_ptr->dest_addr, pk_fd_ptr->dest_addr))
				break;
			}

		/* If we have not found a record, we must create one.	*/
		if (record_index == num_records)
			{
			/* A new flow can be recorded only if stathandles are present.	*/
			if (ete_stat_data_ptr->next_stat_index == ete_stat_max_dim)
				FOUT;
			
			record_ptr = (IpT_Ete_Stat_Record *) op_prg_mem_alloc (sizeof (IpT_Ete_Stat_Record));
			ip_dispatch_ete_stat_record_init (record_ptr, pk_fd_ptr, OPC_TRUE  /* register new handles */);
			op_prg_list_insert (&ete_stat_data_ptr->table, record_ptr, OPC_LISTPOS_TAIL);
			ete_stat_data_ptr->next_stat_index++;
			}
		}

	/* We have a record for this flow, either from before, or one created just now.	*/
	cur_delay = op_sim_time () - op_pk_creation_time_get (pkptr);
	
	if (ip_dispatch_ete_stat_delay_is_requested ())
		op_stat_write (record_ptr->delay_handle, cur_delay);

	if (ip_dispatch_ete_stat_jitter_is_requested ())
		{
		/* Jitter is calculated as SQRT [(ave_delay - cur_delay)^2].	*/
		oms_stat_sample_add (&record_ptr->stat_data, cur_delay);
		op_stat_write (record_ptr->jitter_handle, oms_stat_point_deviance_obtain (&record_ptr->stat_data, cur_delay));
		}
	FOUT;
	}
...
static void
ip_dispatch_dynamic_assignment_handle ()
	{
	IpT_Dynamic_Assignment_Array	*dynamic_assignment_array_ptr;
	IpT_Dynamic_Assignment			*dynamic_assignment_ptr;
	Evhandle						cur_evhandle;
	int								i;
	
	/** Handler function for received Dynamic Assignment interrupts.  The array of			**/
	/** assignments is installed as the event state.  This function iterates over all		**/
	/** assignments and calls the appropriate handler.  Currently add and remove assignments**/
	/** are permitted for address and prefixes.  Update is not supported.					**/
	/**																						**/
	/** To prevent ip_observer being called on an assignment that is invalid (i.e. adding 	**/
	/** to an interface with an address already, attempted removal of an interface with no 	**/
	/** assigned address) this function will track which add operations were successful.  	**/
	
	FIN (ip_dispatch_dynamic_assignment_handle);
	
	cur_evhandle = op_ev_current ();
	dynamic_assignment_array_ptr = (IpT_Dynamic_Assignment_Array *) op_ev_state (cur_evhandle);
	
	if (dynamic_assignment_array_ptr == OPC_NIL)
		{
		FOUT;
		}
	
	if (IPC_DYNAMIC_LTRACE_ACTIVE)
		{
		char msg0 [128];
		
		sprintf (msg0, "Received %d dynamic assignments from addressor objid %d\n",
			dynamic_assignment_array_ptr->assignment_count, op_ev_src_id (cur_evhandle));
		op_prg_odb_print_major (msg0, OPC_NIL);
		}
	
	for (i = 0; i < dynamic_assignment_array_ptr->assignment_count; i++)
		{
		dynamic_assignment_ptr = dynamic_assignment_array_ptr->assignments [i];
		
		switch (dynamic_assignment_ptr->assignment_type)
			{
			case IpC_Dynamic_Assignment_Addr_Create:	
				ip_dispatch_dynamic_address_create (dynamic_assignment_ptr);
				break;
			
			case IpC_Dynamic_Assignment_Prefix_Create:
				ip_dispatch_dynamic_prefix_create (dynamic_assignment_ptr);
				break;
			
			case IpC_Dynamic_Assignment_Addr_Update:
				/* not implemented */
				break;
			
			case IpC_Dynamic_Assignment_Addr_Remove:
			case IpC_Dynamic_Assignment_Prefix_Remove:			
				ip_dispatch_dynamic_address_remove (dynamic_assignment_ptr);
				break;
		
			case IpC_Dynamic_Assignment_Prefix_Update:
				/* not implemented */
				break;
			
			case IpC_Dynamic_Assignment_Default_Route_Add:
				ip_dispatch_dynamic_default_route_handle (dynamic_assignment_ptr, OPC_TRUE /* insert */);
				break;
			
			case IpC_Dynamic_Assignment_Default_Route_Remove:
				ip_dispatch_dynamic_default_route_handle (dynamic_assignment_ptr, OPC_FALSE /* delete */);
				break;
	...
	/* Free assignment memory */
	ip_dynamic_assignment_array_free (dynamic_assignment_array_ptr);
...
static Compcode
ip_dispatch_dynamic_address_create (IpT_Dynamic_Assignment *assignment_ptr)
	{
	IpT_Interface_Info		*intf_ptr;
	Ipv6T_Interface_Info	*ipv6_intf_ptr;
	InetT_Address			dynamic_addr;
	int						nato_index;
	
	/** Assigns an IP address to the specified interface.  If an IP address already exists  **/
	/** it will be removed prior to adding the new one.  This method only performs basic    **/
	/** validation checks--the interface selected to receive the address is assumed to be   **/
	/** dynamically assigned.																**/
	/**	This function enforces the requirement that no existing dynamic address be on the   **/																	
	/** interface.  An action on an interface w/an existing assignment would be the "update"**/
	/** action (not implemented).  If the interface has an assignment, the add will skipped **/
	/** and an ODB message will be printed													**/
	/** When this funciton completes, the new IP address will be registered on the interface**/
	/** and in all other places in the node model.  Four data structures are updated here.  **/
	/** The ip_observer process is used to inform any other processes must be updated.		**/
	/** Although the calling function will manage the memory of the assignment, the space   **/
	/** for the InetT_Address_Range must be managed here, as it may become a part of the 	**/
	/** interface table, or deleted if an error prevents an assignment from being made.		**/
	
	/** There are four structures modified by this function when adding a new address:		**/
	/**	1.  Update the global IPv6 address stored in Ipv6T_Interface_Info of the interface	**/
	/** 2.  NATO table																		**/
	/** 3.  IP Common Route Table: add entries to the address for both "Local" and "Direct" **/
	/**     (routers only step)																**/
	/** 4.  TPAL Map																		**/
	
	FIN (ip_dispatch_dynamic_address_create);
	
	/* Obtain the IPv6 interface pointer  */
	if (!inet_rte_intf_tbl_index_valid (&module_data, assignment_ptr->intf_index))
		{
		FRET (OPC_COMPCODE_FAILURE);
		}	
	intf_ptr = inet_rte_intf_tbl_access (&module_data, assignment_ptr->intf_index);
	ipv6_intf_ptr = intf_ptr->ipv6_info_ptr;
	
        if (ipv6_intf_ptr == OPC_NIL)
		{
		op_sim_error (OPC_SIM_ERROR_RECOVER, "ip_dispatch unable to locate IPv6T_Interface data for",
			"the specified dynamic interface.");
		FRET (OPC_COMPCODE_FAILURE);
		}
	else if (!ipv6_intf_ptr->dhcp_enabled)
	        {
		FRET (OPC_COMPCODE_FAILURE);
		}
	else if (ipv6_intf_ptr->num_addresses != 1)
		{
		/* If a dynamic address already exists, remove it before adding a new one */
		/* This is possible if the DHCP server changes */
		ip_dispatch_dynamic_address_remove (assignment_ptr);
		}
	
	/* Get a local copy of the dynamic address from the address range */
	dynamic_addr = inet_address_range_addr_get_fast (&(assignment_ptr->dynamic_addr_range));
	
	/* An existing address should not exist (see note in function header).  Skip this assignment and print ODB message */
	if (inet_address_valid ((ipv6_intf_ptr->ipv6_addr_array) [1]))
		{
		op_prg_odb_print_major ("DHCP attempting to add an address to an interface with a dynamic",
			"address already configured.  Skipping the assignment and using the old", 
			OPC_NIL);
	...
	/* 1.  Update the global IPv6 address stored in Ipv6T_Interface_Info of the interface	*/
	(ipv6_intf_ptr->ipv6_addr_array) [1] = inet_address_range_copy (assignment_ptr->dynamic_addr_range);
	ipv6_intf_ptr->num_addresses++;
	
	/* 2.  NATO table																		*/
	/* Store the new address in the NATO table and save references to it */
	ip_rtab_local_addr_register (&dynamic_addr, &module_data);
	nato_index = nato_table_inet_address_to_index (ip_table_handle, &dynamic_addr, 0);
	
	/* Register the physical address back into the NATO table, reading from the interface ptr */
	/* Only supported for Ethernet LAN at the moment */
	if (intf_ptr->phys_intf_info_ptr->lower_layer_type == IpC_Intf_Lower_Layer_LAN)
		{
		nato_table_lower_layer_address_register (ip_table_handle, nato_index,
			intf_ptr->phys_intf_info_ptr->lower_layer_addr, NATOC_LOWER_LAYER_ADDR_TYPE_0);
		}
	
	/* 3a.  IP Common Route Table: add entries to the address for both "Local" and "Direct" 	*/
	/*  b.  Add entry in prefix list for RA */
	/* This step is for routers only */
	if (ip_rte_node_is_gateway (&module_data))
		{
		Ipv6T_Ra_Gtwy_Info *ra_info;
		
		/* Only add the addresses into the routing table if the link is active */
		if (intf_ptr->phys_intf_info_ptr->link_status == 1)
			{
			ip_directly_connected_networks_for_interface_handle (intf_ptr, assignment_ptr->intf_index, OPC_TRUE);
			}
		
		ra_info = ipv6_intf_ptr->ra_info.ra_gtwy_info_ptr;
		ra_info->adv_prefix_list [ra_info->num_adv_prefixes].addr_prefix = inet_address_range_copy (assignment_ptr->dynamic_addr_range);
		ra_info->num_adv_prefixes++;
		}
	
	
	/* 4.  TPAL Map																			*/
	tpal_ip_address_add (module_data.node_id, dynamic_addr, OPC_TRUE /* make primary addr */);
...
static Compcode
ip_dispatch_dynamic_prefix_create (IpT_Dynamic_Assignment *assignment_ptr)
	{
	IpT_Interface_Info		*intf_ptr;

	/* A prefix address assignment must first be modified to have its lower bits set according */
	/* to the interface ID.  Do this before calling ip_dispatch_dynamic_address_create */
	
	FIN (ip_dispatch_dynamic_prefix_create);
	
	/* Obtain the Interface pointers */
	if (!inet_rte_intf_tbl_index_valid (&module_data, assignment_ptr->intf_index))
		{
		FRET (OPC_COMPCODE_FAILURE);
		}
	...
	assignment_ptr->dynamic_addr_range.address.ipv6_addr_ptr->addr32 [3] = intf_ptr->ipv6_info_ptr->intf_id;

	FRET (ip_dispatch_dynamic_address_create (assignment_ptr));
	}

static Compcode
ip_dispatch_dynamic_address_remove (IpT_Dynamic_Assignment *assignment_ptr)
	{
	IpT_Interface_Info		*intf_ptr;
	Ipv6T_Interface_Info	*ipv6_intf_ptr;
	InetT_Address			dynamic_addr;
	...
	/** This function removes a dynamic address assigned on the specified interface. The 	**/
	/** function will have already verified that the interface contains a dynamic address	**/
	/** that is to be removed.																**/
	/** The four steps of adding an address are done in reverse order on removal:			**/
	/** 1.  TPAL Map																		**/
	/** 2.  IP Common Route Table: entries to the address for both "Local" and "Direct" 	**/
	/**     (routers only step)																**/	
	/** 3.  Clear out NATO table															**/
	/**	4.  Update the global IPv6 address stored in Ipv6T_Interface_Info of the interface	**/
	
	FIN (ip_dispatch_dynamic_address_remove);
	...
	if (ipv6_intf_ptr == OPC_NIL ||
	    !ipv6_intf_ptr->dhcp_enabled)
	    FRET (OPC_COMPCODE_FAILURE);

	/* Get the address from the interface */
	dynamic_addr = inet_address_range_addr_get_fast (&(ipv6_intf_ptr->ipv6_addr_array [1]));
	
	/* 1. Remove TPAL */
	tpal_dynamic_ip_address_remove (module_data.node_id, dynamic_addr);
	
	/* 2a. Remove Common Route Table entries if this node is a router */
	/*  b. Remove gateway prefix advertised in RA */
	if (ip_rte_node_is_gateway (&module_data))
		{
		Ipv6T_Ra_Gtwy_Info *ra_info;
		...
		if (intf_ptr->phys_intf_info_ptr->link_status == 1)
			{
			ip_directly_connected_networks_for_interface_handle (intf_ptr, assignment_ptr->intf_index, OPC_FALSE);
			}
		
		ra_info = ipv6_intf_ptr->ra_info.ra_gtwy_info_ptr;
		ra_info->num_adv_prefixes--;
		inet_address_range_destroy (&(ra_info->adv_prefix_list [ra_info->num_adv_prefixes].addr_prefix));
		ra_info->adv_prefix_list [ra_info->num_adv_prefixes].addr_prefix = InetI_Addr_Range_Invalid;
		}
	
	/* 3. Clear out the NATO Table */
	/* Do so by scrubbing out the entry's physical address bindings */
	nato_index = nato_table_inet_address_to_index (ip_table_handle, &dynamic_addr, 0);	
	for (i = 0; i < NATOC_NUM_LOWER_LAYER_ADDRS; i++)
		{
		nato_table_lower_layer_address_register (ip_table_handle, nato_index,
			NATOC_LOWER_LAYER_ADDR_UNDEFINED, i);
		}
	
	/* 4. Replace the global address */
	(ipv6_intf_ptr->ipv6_addr_array) [1] = InetI_Addr_Range_Invalid;
	ipv6_intf_ptr->num_addresses--;
...
static Compcode				
ip_dispatch_dynamic_default_route_handle (IpT_Dynamic_Assignment *assignment_ptr, Boolean insert)
	{
	...
	/* In DHCP Prefix delegation, the requesting router must install a default 	*/
	/* route to the delegating router so that it can connect to the rest of the */
	/* network. */
	
	FIN (ip_dispatch_dynamic_default_route_handle);
	
	intf_ptr = inet_rte_intf_tbl_access (&module_data, assignment_ptr->intf_index);
	addr_range = inet_address_range_create (InetI_Default_v6_Addr, 0);
	dest_address = inet_address_range_addr_get (&assignment_ptr->dynamic_addr_range);
	port_info = ip_rte_port_info_create (assignment_ptr->intf_index, intf_ptr->full_name);
	
	if (insert)
		{
		Inet_Cmn_Rte_Table_Entry_Add (module_data.ip_route_table,
			(void *) OPC_NIL,
			addr_range,
			dest_address,
			port_info,
			0, // metric
			IP_CMN_RTE_TABLE_UNIQUE_ROUTE_PROTO_ID (IpC_Dyn_Rte_Static, IPC_NO_MULTIPLE_PROC), // proto 
			0); // admin distance
		}
	else
		{
		Inet_Cmn_Rte_Table_Entry_Delete (module_data.ip_route_table,
			addr_range,
			dest_address,
...
static Boolean					
ip_dispatch_intf_is_active (Objid intf_objid)
	{
	Boolean			status;
	int				operational_status;
	
	/** Read the status and operational status of an attribute and return 	**/
	/** a YES/NO answer. Status may be active or shutdown. Operational 		**/
	/** status may be up/down/infer. For DES, Up and Infer are the same.	**/
	FIN (ip_dispatch_intf_is_active ());

	op_ima_obj_attr_get_toggle (intf_objid, "Status", &status);

	/* Operational status is relevant only if the status is enabled/active.	*/
	if (status == OPC_BOOLINT_ENABLED)
		{
		op_ima_obj_attr_get_int32 (intf_objid, "Operational Status", &operational_status);

		/* Up and Infer mean Up in DES. Down means down.	*/
		if (operational_status == IPC_INTF_OPERATIONAL_STATUS_DOWN)
			status = OPC_BOOLINT_DISABLED;
		}
...
extern "C" {
	...
	VosT_Address _op_ip_dispatch_smf_alloc (VosT_Obtype, int);
	...
	FIN_MT (ip_dispatch_smf ());
		...
		FSM_BLOCK_SWITCH
			...
			FSM_TRANSIT_SWITCH
				...
				FSM_CASE_TRANSIT (4, 4, state4_enter_exec, ip_dispatch_dynamic_assignment_handle ();, "DYN_ASSIGNMENT", "ip_dispatch_dynamic_assignment_handle ()", "idle", "idle", "ip_dispatch_smf [idle -> idle : DYN_ASSIGNMENT / ip_dispatch_dynamic_assignment_handle ()]")
				}
				/*---------------------------------------------------------*/
...
_op_ip_dispatch_smf_diag (OP_SIM_CONTEXT_ARG_OPT)
	...
	if (1)
		...
		if (is_ip_initialized == OPC_FALSE)
			...
			if (op_prg_odb_ltrace_active ("ip_frag"))
				{
				ip_frag_sup_print (module_data.rsm_buf_ptr);
				}	
			...
			if (op_prg_odb_ltrace_active ("ip_redist"))
				ip_cmn_rte_table_redist_matrix_print (module_data.ip_route_table);
...
#undef observer_prohandle
#undef dynamic_addr_nato_key_lptr
#undef ete_stat_data_ptr
...
VosT_Address
_op_ip_dispatch_smf_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	...
	ptr = (ip_dispatch_smf_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
...
_op_ip_dispatch_smf_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	...
	if (strcmp ("observer_prohandle" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->observer_prohandle);
		FOUT
		}
	if (strcmp ("dynamic_addr_nato_key_lptr" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->dynamic_addr_nato_key_lptr);
		FOUT
		}
	if (strcmp ("ete_stat_data_ptr" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->ete_stat_data_ptr);
		FOUT
		}
	*var_p_ptr = (void *)OPC_NIL;
