...
typedef enum
	...
	IpC_Rte_Aodv,
	IpC_Rte_Olsr,
	IpC_Rte_OLSR_NRL,   // JPH 9/6/2006 - added line to 22 Aug 2006 version
	IpC_Rte_Ripng,
	IpC_Rte_Ospf3,
	IpC_Rte_Static
	} IpT_Rte_Protocol;
...
#define IPC_RTE_PROTO_DSR  		(1<<8)
#define IPC_RTE_PROTO_TORA		(1<<9)
#define IPC_RTE_PROTO_AODV		(1<<10)
#define	IPC_RTE_PROTO_OLSR		(1<<11)
...
typedef struct Ipv6T_Interface_Info
	...
	int								num_addresses;		/* Number of IPv6 addresses (incl. the link	*/
														/* local address).							*/
	Boolean							dhcp_enabled;		/* Set to true if DHCP is to configure the  */
...
void ip_rtab_lower_layer_address_register (InetT_Address addr, int lower_layer_address, int lower_layer_addr_type);
#if defined (__cplusplus)
} /* end of 'extern "C" {' */
#endif
