/* Process model C++ form file: olsr_protolib.pr.cpp */
/* Portions of this file copyright 1992-2004 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char olsr_protolib_pr_cpp [] = "MIL_3_Tfile_Hdr_ 115A 30A modeler 7 43E7BF78 43E7BF78 1 apocalypse Jim@Hauser 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 d18 1                                                                                                                                                                                                                                                                                                                                                                                                     ";
#include <string.h>
...
#define IP_NOTIFICATION			op_intrpt_type () == OPC_INTRPT_REMOTE
...
class olsr_protolib_state
	{
	public:
		olsr_protolib_state (void);
		...
		/* FSM code */
		void olsr_protolib (OP_SIM_CONTEXT_ARG_OPT);
		/* Diagnostic Block */
		void _op_olsr_protolib_diag (OP_SIM_CONTEXT_ARG_OPT);
...
VosT_Obtype olsr_protolib_state::obtype = (VosT_Obtype)OPC_NIL;

#define pr_state_ptr            		((olsr_protolib_state*) (OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))
...
#  define FIN_PREAMBLE_DEC	olsr_protolib_state *op_sv_ptr;
#if defined (OPD_PARALLEL)
#  define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((olsr_protolib_state *)(sim_context_ptr->_op_mod_state_ptr));
#else
...
{   
   ...
   own_process_record_handle = oms_pr_process_register(own_node_objid, 
        own_module_objid, own_prohandle, proc_model_name);

    // Register the protocol attribute in the registry. */
    oms_pr_attr_set(own_process_record_handle, "protocol", OMSC_PR_STRING, "OLSR_NRL", OPC_NIL);
...
void OLSR_startup()
{
   ...
   while (op_prg_list_size(proc_record_handle_list_ptr) > 0)
    ...
    else
	{
	    ...
	    oms_pr_attr_get(process_record_handle, "module data", OMSC_PR_ADDRESS, &my_rte_module_data); 
	}
	...
	//* Get Interface Name required initializing routing table first */
	...
	// Schedule interrupt for end of simulation.	
    op_intrpt_schedule_call (OPC_INTRPT_SCHED_CALL_ENDSIM, OPC_INT_UNDEF, olsr_rte_route_table_to_file_export, OPC_NIL); // LP 9-20-05 added */
...
extern "C"
{
	VosT_Obtype _op_olsr_protolib_init (int * init_block_ptr);
	VosT_Address _op_olsr_protolib_alloc (VOS_THREAD_INDEX_ARG_COMMA VosT_Obtype, int);
	void olsr_protolib (OP_SIM_CONTEXT_ARG_OPT)
		{
		((olsr_protolib_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->olsr_protolib (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_olsr_protolib_svar (void *, const char *, void **);

	void _op_olsr_protolib_diag (OP_SIM_CONTEXT_ARG_OPT)
		{
		((olsr_protolib_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->_op_olsr_protolib_diag (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_olsr_protolib_terminate (OP_SIM_CONTEXT_ARG_OPT)
		{
		/* The destructor is the Termination Block */
		delete (olsr_protolib_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr);
		}
...
void
olsr_protolib_state::olsr_protolib (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (olsr_protolib_state::olsr_protolib ());
	try
		{
		/* Temporary Variables */
		Packet*				pkptr;
		...
		*/
		...
		FSM_ENTER ("olsr_protolib")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (proc_msg) enter executives **/
			FSM_STATE_ENTER_FORCED (0, "proc_msg", state0_enter_exec, "olsr_protolib [proc_msg enter execs]")
				FSM_PROFILE_SECTION_IN ("olsr_protolib [proc_msg enter execs]", state0_enter_exec)
				{
#if OP_DEBUG1
			...
			FSM_STATE_EXIT_FORCED (0, "proc_msg", "olsr_protolib [proc_msg exit execs]")


			/** state (proc_msg) transition processing **/
			FSM_TRANSIT_FORCE (2, state2_enter_exec, ;, "default", "", "proc_msg", "idle", "olsr_protolib [proc_msg -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (init) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "init", state1_enter_exec, "olsr_protolib [init enter execs]")
				FSM_PROFILE_SECTION_IN ("olsr_protolib [init enter execs]", state1_enter_exec)
				{
			...
			FSM_EXIT (3,"olsr_protolib")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "init", "olsr_protolib [init exit execs]")
				FSM_PROFILE_SECTION_IN ("olsr_protolib [init exit execs]", state1_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("olsr_protolib [init trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "IP_NOTIFICATION", "", "init", "idle", "olsr_protolib [init -> idle : IP_NOTIFICATION / ]")
				FSM_CASE_TRANSIT (1, 1, state1_enter_exec, ;, "default", "", "init", "init", "olsr_protolib [init -> init : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "idle", state2_enter_exec, "olsr_protolib [idle enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"olsr_protolib")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "idle", "olsr_protolib [idle exit execs]")


			/** state (idle) transition processing **/
			FSM_PROFILE_SECTION_IN ("olsr_protolib [idle trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 0, state0_enter_exec, ;, "MESSAGE_RECEIVED", "", "idle", "proc_msg", "olsr_protolib [idle -> proc_msg : MESSAGE_RECEIVED / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "TIMEOUT_EVENT", "", "idle", "itimer", "olsr_protolib [idle -> itimer : TIMEOUT_EVENT / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "PRINT_RT_EVENT", "", "idle", "print_rt", "olsr_protolib [idle -> print_rt : PRINT_RT_EVENT / ]")
				FSM_CASE_TRANSIT (3, 5, state5_enter_exec, ;, "END_SIM", "", "idle", "end_sim", "olsr_protolib [idle -> end_sim : END_SIM / ]")
				FSM_CASE_TRANSIT (4, 2, state2_enter_exec, ;, "default", "", "idle", "idle", "olsr_protolib [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (3, "itimer", state3_enter_exec, "olsr_protolib [itimer enter execs]")
				FSM_PROFILE_SECTION_IN ("olsr_protolib [itimer enter execs]", state3_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "itimer", "olsr_protolib [itimer exit execs]")


			/** state (itimer) transition processing **/
			FSM_TRANSIT_FORCE (2, state2_enter_exec, ;, "default", "", "itimer", "idle", "olsr_protolib [itimer -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (print_rt) enter executives **/
			FSM_STATE_ENTER_FORCED (4, "print_rt", state4_enter_exec, "olsr_protolib [print_rt enter execs]")
				FSM_PROFILE_SECTION_IN ("olsr_protolib [print_rt enter execs]", state4_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (4, "print_rt", "olsr_protolib [print_rt exit execs]")


			/** state (print_rt) transition processing **/
			FSM_TRANSIT_FORCE (2, state2_enter_exec, ;, "default", "", "print_rt", "idle", "olsr_protolib [print_rt -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (end_sim) enter executives **/
			FSM_STATE_ENTER_FORCED (5, "end_sim", state5_enter_exec, "olsr_protolib [end_sim enter execs]")
				FSM_PROFILE_SECTION_IN ("olsr_protolib [end_sim enter execs]", state5_enter_exec)
				{
				printf("****************************************************************\n");
				printf("*                                                              *\n");
			...
			FSM_STATE_EXIT_FORCED (5, "end_sim", "olsr_protolib [end_sim exit execs]")


			/** state (end_sim) transition processing **/
			FSM_TRANSIT_FORCE (2, state2_enter_exec, ;, "default", "", "end_sim", "idle", "olsr_protolib [end_sim -> idle : default / ]")
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (1,"olsr_protolib")
		}
	catch (...)
		{
		Vos_Error_Print (VOSC_ERROR_ABORT,
			(const char *)VOSC_NIL,
			"Unhandled C++ exception in process model (olsr_protolib)",
...
void
olsr_protolib_state::_op_olsr_protolib_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	...
	FIN_MT (olsr_protolib_state::_op_olsr_protolib_diag ())
...
void
olsr_protolib_state::operator delete (void* ptr)
	{
	FIN (olsr_protolib_state::operator delete (ptr));
	Vos_Poolmem_Dealloc_MT (OP_SIM_CONTEXT_THREAD_INDEX_COMMA ptr);
	FOUT
	}

olsr_protolib_state::~olsr_protolib_state (void)
	{

	FIN (olsr_protolib_state::~olsr_protolib_state ())
...
void *
olsr_protolib_state::operator new (size_t)
#if defined (VOSD_NEW_BAD_ALLOC)
	...
	FIN_MT (olsr_protolib_state::operator new ());

	new_ptr = Vos_Alloc_Object_MT (VOS_THREAD_INDEX_UNKNOWN_COMMA olsr_protolib_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
olsr_protolib_state::olsr_protolib_state (void) :
	...
	{
#if defined (OPD_ALLOW_ODB)
		_op_current_state = "olsr_protolib [init enter execs]";
#endif
	}

VosT_Obtype
_op_olsr_protolib_init (int * init_block_ptr)
	{
	FIN_MT (_op_olsr_protolib_init (init_block_ptr))

	olsr_protolib_state::obtype = Vos_Define_Object_Prstate ("proc state vars (olsr_protolib)",
		sizeof (olsr_protolib_state));
	*init_block_ptr = 2;

	FRET (olsr_protolib_state::obtype)
	}

VosT_Address
_op_olsr_protolib_alloc (VOS_THREAD_INDEX_ARG_COMMA VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	olsr_protolib_state * ptr;
	FIN_MT (_op_olsr_protolib_alloc ())

	/* New instance will have FSM handling initialized */
#if defined (VOSD_NEW_BAD_ALLOC)
	try {
		ptr = new olsr_protolib_state;
...
void
_op_olsr_protolib_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	olsr_protolib_state		*prs_ptr;

	FIN_MT (_op_olsr_protolib_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (olsr_protolib_state *)gen_ptr;
