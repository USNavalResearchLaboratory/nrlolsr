...
const char olsr_protolib_smf_pr_cpp [] = "MIL_3_Tfile_Hdr_ 120A 30A op_runsim 7 464DAC99 464DAC99 1 wn12jh Jim@Hauser 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 10de 3                                                                                                                                                                                                                                                                                                                                                                                                      ";
#include <string.h>
...
//  JPH - animation erasure of collision drawing draw by dra_inoise_col pipeline stage
#define COL_TO				(op_intrpt_type () == OPC_INTRPT_REMOTE && (op_intrpt_code() == COL_ANIM_TO))
...
void anim_pos_update(int); /* JPH animation */
...
/* JPH 10/5/2006 - animation development */


static char icon_type[12][8] =
{
	{"sta"},
	{"mpr"},
	{"mpr2"},
...
int animFlag = OPC_FALSE;
int sdtFlag = OPC_FALSE;
int no_coll_anim = OPC_FALSE;
Anvid olsr_vid;
OlsrNode* olsrNode[MAX_NUM_NODES];
Andid col_did[MAX_NUM_NODES];

static int pnum = 0;
static int numOlsrCapablePlatformsInSimulation = 0;
static OlsrLink* olsrLink[MAX_NUM_NODES][MAX_NUM_NODES];
static addrmap[256]; // map ipv4 addr (last byte) to pnum for accessing olsrNode via ipv4 addr
static int animInit = OPC_FALSE;
static char network_name[256];
static char sdt_filename[256];
static FILE* sdtFile = NULL;

OlsrNode::OlsrNode()
{ 
  objid = OPC_OBJID_INVALID;	// use to detect if object exists for this data structure
  col_did = -1;
  col_cnt = 0;
  col_exception_cnt = 0;
  lat = 0.0;
...
OlsrLink::OlsrLink()
{	
  did = -1; // Invalid drawing id
  linkType = NO_LINK;
}
	
/* end JPH 10/5/2006 - animation development */
...
class olsr_protolib_smf_state
	...
	public:
		...
		int	                    		my_pnum                                         ;	/* JPH animation - order in which this OLSR-capable node was created. */
		int	                    		mprOrder                                        ;	/* JPH animation - How many neighbors have selected this node as an mpr? */
		Andid	                  		node_did                                        ;	/* JPH animation - drawing id for surrounding node */
		Andid	                  		node_name_did                                   ;	/* JPH animation */
		Andid	                  		pk_collision_did                                ;	/* JPH animation */
		char	                   		my_node_name[64]                                ;	/* JPH animation */
		IpT_Address	            		my_ip_addr                                      ;	/* JPH animation */
		IpT_Address	            		my_ip_mask                                      ;	/* JPH animation */
		ProtoAddress	           		host_ipv4_addr                                  ;
		int	                    		no_link_anim                                    ;	/* JPH animation */
		int	                    		no_node_anim                                    ;	/* JPH animation */
...
#define mpr_list_sent_stathandle		op_sv_ptr->mpr_list_sent_stathandle
#define num_hello_pk_sent_stathandle		op_sv_ptr->num_hello_pk_sent_stathandle
#define num_TC_pk_sent_stathandle		op_sv_ptr->num_TC_pk_sent_stathandle
#define MPR_status_stathandle   		op_sv_ptr->MPR_status_stathandle
#define NODE_ID                 		op_sv_ptr->NODE_ID
#define own_olsr_process        		op_sv_ptr->own_olsr_process
...
#define interface_info_pnt      		op_sv_ptr->interface_info_pnt
#define common_routing_table    		op_sv_ptr->common_routing_table
#define olsr_port_info          		op_sv_ptr->olsr_port_info
#define udp_command_ici_ptr     		op_sv_ptr->udp_command_ici_ptr
#define my_udp_objid            		op_sv_ptr->my_udp_objid
#define my_smf_objid            		op_sv_ptr->my_smf_objid
#define my_rte_module_data      		op_sv_ptr->my_rte_module_data
#define move_process_hndl       		op_sv_ptr->move_process_hndl
#define smf_support             		op_sv_ptr->smf_support
#define num_hello_pk_sent       		op_sv_ptr->num_hello_pk_sent
#define num_hello_pk_rcv        		op_sv_ptr->num_hello_pk_rcv
#define num_TC_pk_sent          		op_sv_ptr->num_TC_pk_sent
#define num_TC_pk_rcv           		op_sv_ptr->num_TC_pk_rcv
#define my_pnum                 		op_sv_ptr->my_pnum
#define mprOrder                		op_sv_ptr->mprOrder
#define node_did                		op_sv_ptr->node_did
#define node_name_did           		op_sv_ptr->node_name_did
#define pk_collision_did        		op_sv_ptr->pk_collision_did
#define my_node_name            		op_sv_ptr->my_node_name
...
#define no_link_anim            		op_sv_ptr->no_link_anim
#define no_node_anim            		op_sv_ptr->no_node_anim
...
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((olsr_protolib_smf_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
{   
	...
	/* Obtain the name attribute of the surrounding node */
	op_ima_obj_attr_get (own_node_objid, "name", my_node_name);
	...
	op_ima_obj_attr_get_toggle(own_module_objid,"SMF Support",&smf_support);
  	...
  	/* Get value of toggle which indicates if link animation is turned off */  /* JPH ANIM */
	op_ima_sim_attr_get(OPC_IMA_TOGGLE,"Link Anim OFF",&no_link_anim);
	
  	/* Get value of toggle which indicates if node animation is turned off */  /* JPH ANIM */
	op_ima_sim_attr_get(OPC_IMA_TOGGLE,"Node Anim OFF",&no_node_anim);
	
  	/* Get value of toggle which indicates if collision animation is turned off */  /* JPH ANIM */
	op_ima_sim_attr_get(OPC_IMA_TOGGLE,"Collision Anim OFF",&no_coll_anim);
...
// JPH SMF
	...
	/* JPH 10-11-2006  animation */
	numOlsrCapablePlatformsInSimulation++;
	mprOrder = 0;
	node_did = -1;
	node_name_did = -1;
	/* end JPH 10-11-2006  animation */
...
void OLSR_startup()
{
	...
	*/
	...
	if (animFlag) anim_pos_update(my_pnum);  // JPH animation - initial update of my node's position
	...
	*/
	...
	/* JPH 9/15/2006 - Process command line input */
	if (op_ima_sim_attr_exists("OLSR Command Line"))
	{
		char cmdline[512];
		op_ima_sim_attr_get(OPC_IMA_STRING,"OLSR Command Line",cmdline);
		if (cmdline[0])
			own_olsr_process.Nrlolsr::StringProcessCommands(cmdline);
	}
	/* JPH 9/15/2006 - End Process command line input */
	...
	*/
	...
	if (animFlag)
		...
		olsrNode[my_pnum]->ipv4_addr = host_ipv4_addr;
		/* At some point, replace following with hash table approach. For now, use last byte (host addr) of ipv4 addr as indice for OlsrNode. */
		int fld1,fld2,fld3,fld4;
		ip_address_fields_obtain(my_ip_addr,&fld1,&fld2,&fld3,&fld4);
		addrmap[fld4] = my_pnum;
		}
	/* end JPH animation */
	...
	*/
    IpT_Address broadcastAddr = 
        ip_address_node_broadcast_create(my_ip_addr, my_ip_mask);
	...
	/* Get Log File Name */
	if (logFileFlag)  // Log File is global, so set only once.
		{
	...
	/* Get Debug Level */
	//if (debugFlag) // Debug Level is global, so set only once.
		{
		debugFlag = 0;
...
// end LP
			...
			/* JPH animation */
			// update own position in viewer
			if (animFlag || sdtFlag)
				{
				NBRQueue mprlist = own_olsr_process.mprSelectorList;
				NBRQueue nbrlist = own_olsr_process.nbr_list;
				int numRoutes = own_olsr_process.getNumRoutes();
				double routeRatio = (double)numRoutes/(pnum-1);
				int connectedness = 0;
				if (routeRatio >= .9)
					connectedness = 4;
				if (routeRatio == 1.0)
				...
				mprOrder = 0;
				if (!mprlist.IsEmpty())
					for (NbrTuple *nb=mprlist.PeekInit();nb!=NULL;nb=mprlist.PeekNext())
						{
						mprOrder++;
						if (mprOrder >2)
							break;
				...
				anim_pos_update(my_pnum);  // update my node's position
				
				if (animFlag)
					{
					/* Erase node's icon at previous position. */
					if (node_did > -1)
						op_anim_igp_drawing_erase (olsr_vid, node_did, OPC_ANIM_ERASE_MODE_XOR);

					/* Redraw node's icon at new position. */
					node_did = op_anim_igp_icon_draw (olsr_vid, OPC_ANIM_RETAIN | OPC_ANIM_PIXOP_XOR, 
						icon_type[connectedness+mprOrder], olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy); 

					/* Erase node's name text at previous position. */
					if (node_name_did > -1)
						op_anim_igp_drawing_erase (olsr_vid, node_name_did, OPC_ANIM_ERASE_MODE_XOR);

					/* Redraw node's name text at new position. */
					node_name_did = op_anim_igp_text_draw (olsr_vid, 
						OPC_ANIM_RETAIN | OPC_ANIM_PIXOP_XOR |
						OPC_ANIM_ALIGNH_CENTER | OPC_ANIM_ALIGNV_CENTER,
						olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy-15, my_node_name);
					}
				
				if (sdtFlag)
					{
					fprintf(sdtFile,"node %s type %s pos %d,%d\n",my_node_name,
						icon_type[connectedness+mprOrder],
						olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy);
					}
				
				/* Check to see if link animation is turned on. */
				if (!no_link_anim)
					{
				
					/* Erase links drawn by this node before redrawing the current links */
					for (int i=0; i<pnum; i++)
						if (my_pnum != i)
							{
							olsrLink[my_pnum][i]->linkType = NO_LINK;
							if (olsrLink[my_pnum][i]->did >= 0)  // See if old link that this node drew needs to be erased
								{
								if (animFlag)
									op_anim_igp_drawing_erase (olsr_vid, olsrLink[my_pnum][i]->did, OPC_ANIM_ERASE_MODE_XOR);
								if (sdtFlag)
									fprintf(sdtFile,"unlink %s,%s\n",my_node_name,olsrNode[i]->name);
								olsrLink[my_pnum][i]->did = -1;  // Reset did if link is erased
								//printf ("mpr my drawing_erase: %d %d %d\n",my_pnum,n2,olsrLink[my_pnum][i]->did);
								}					
							}
					...
					if (!mprlist.IsEmpty())
						for (NbrTuple *nb=mprlist.PeekInit();nb!=NULL;nb=mprlist.PeekNext())
							{
							int sym_props = OPC_ANIM_RETAIN | OPC_ANIM_PIXOP_XOR | OPC_ANIM_ALIGNH_CENTER | OPC_ANIM_ALIGNV_CENTER | OPC_ANIM_STYLE_SOLID;  // Solid lines for mpr links
							int fld1,fld2,fld3,fld4;
							int link_color;
							ip_address_fields_obtain(nb->N_addr.SimGetAddress(),&fld1,&fld2,&fld3,&fld4);
							int n2 = addrmap[fld4];  // index of neighboring node in mpr list
							olsrLink[my_pnum][n2]->linkType = MPR_LINK; // Record link status
							if (olsrLink[n2][my_pnum]->did >= 0)  // See if old link that neighboring node drew needs to be erased
								{
								if (animFlag)
									op_anim_igp_drawing_erase (olsr_vid, olsrLink[n2][my_pnum]->did, OPC_ANIM_ERASE_MODE_XOR);
								if (sdtFlag)
									fprintf(sdtFile,"unlink %s,%s\n",olsrNode[n2]->name,my_node_name);
								olsrLink[n2][my_pnum]->did = -1;  // Reset did if link is erased
								}
							if (olsrLink[n2][my_pnum]->linkType == MPR_LINK)
								{
								sym_props = sym_props | OPC_ANIM_COLOR_RGB222;  // mpr link in both directions
								link_color = GREEN;  // green color for bi-directional link
								}
							else
								{
								sym_props = sym_props | OPC_ANIM_COLOR_RGB122;  //  mpr in single direction
								link_color = RED;  // red color for uni-directional link
								}
							if (animFlag)
								olsrLink[my_pnum][n2]->did = op_anim_igp_line_draw (olsr_vid, sym_props,
									olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy, olsrNode[n2]->vx, olsrNode[n2]->vy);
							if (sdtFlag)
								{
								fprintf(sdtFile,"link %s,%s,%s,2\n",my_node_name,olsrNode[n2]->name,color[link_color]); // SDT line thickness = 2 for mpr links
								if (!animFlag)
									olsrLink[my_pnum][n2]->did = 1;  // fake did if not using Opnet animation
								}
							}

					if (!nbrlist.IsEmpty())
						for (NbrTuple *nb=nbrlist.PeekInit();nb!=NULL;nb=nbrlist.PeekNext())
							{						
							int sym_props = OPC_ANIM_RETAIN | OPC_ANIM_PIXOP_XOR | OPC_ANIM_ALIGNH_CENTER | OPC_ANIM_ALIGNV_CENTER;
							int fld1,fld2,fld3,fld4;
							int link_color;
							int link_width;
							ip_address_fields_obtain(nb->N_addr.SimGetAddress(),&fld1,&fld2,&fld3,&fld4);
							int n2 = addrmap[fld4];  // index of neighboring node in mpr list
							if (olsrLink[my_pnum][n2]->linkType == MPR_LINK)
								continue;  // If nbr is an mpr, drawing has already been done by mpr loop above.
							olsrLink[my_pnum][n2]->linkType = LINK; // Record link status
							if (olsrLink[n2][my_pnum]->did >= 0)  // See if old link that neighboring node drew needs to be erased
								{
								if (animFlag)
									op_anim_igp_drawing_erase (olsr_vid, olsrLink[n2][my_pnum]->did, OPC_ANIM_ERASE_MODE_XOR);
							...
							if (olsrLink[n2][my_pnum]->linkType == LINK)
								{
								sym_props = sym_props | OPC_ANIM_COLOR_RGB222 | OPC_ANIM_STYLE_DASHES;  // nbr in both directions
								link_color = GREEN;  // green color for bi-directional link
								link_width = 1;  // nbr link
								}
							else if (olsrLink[n2][my_pnum]->linkType == MPR_LINK)
								{
								sym_props = sym_props | OPC_ANIM_COLOR_RGB122 | OPC_ANIM_STYLE_SOLID;  // mpr in single direction
								link_color = RED;  // red color for uni-directional link
								link_width = 2;  // mpr link
								}
							else
								{
								sym_props = sym_props | OPC_ANIM_COLOR_RGB122 | OPC_ANIM_STYLE_DASHES;  // nbr in single direction
								link_color = RED;  // red color for uni-directional link
								link_width = 1;  // nbr link
								}
							if (animFlag)
							...
							if (sdtFlag)
								{
								fprintf(sdtFile,"link %s,%s,%s,%d\n",my_node_name,olsrNode[n2]->name,color[link_color],link_width); // SDT line thickness = 2 for mpr links
								if (!animFlag)
									olsrLink[my_pnum][n2]->did = 1;  // fake did if not using Opnet animation
								}
							}
...
void anim_pos_update (int n)
{
	FIN (anim_pos_update(n));
	/** Mobile node position update interrupt: animate node's new position. **/ 
	/* Force calculation of new node position. */
	op_ima_obj_pos_get (olsrNode[n]->objid, &olsrNode[n]->lat, &olsrNode[n]->lon, &olsrNode[n]->alt,
		&olsrNode[n]->x, &olsrNode[n]->y, &olsrNode[n]->z);

	/* Obtain new position of node in subnet coordinates. */
	op_ima_obj_attr_get (olsrNode[n]->objid, "x position", &(olsrNode[n]->x));
	op_ima_obj_attr_get (olsrNode[n]->objid, "y position", &(olsrNode[n]->y));

	/* Convert node position from model coordinates to animation viewer coordinates. */
	op_anim_ime_gen_pos (olsr_vid, olsrNode[n]->x, olsrNode[n]->y, &(olsrNode[n]->vx), &(olsrNode[n]->vy));
	FOUT;
}

void Nrlolsr::linkDown (ProtoAddress addr)
{
	FIN(linkDown (addr));
	if (animFlag)
		{
		int fld1,fld2,fld3,fld4;
		ip_address_fields_obtain(addr.SimGetAddress(),&fld1,&fld2,&fld3,&fld4);
		int n2 = addrmap[fld4];  // index of neighboring node with link down
		olsrLink[my_pnum][n2]->linkType = NO_LINK;
		if (olsrLink[my_pnum][n2]->did >= 0)
			{
			op_anim_igp_drawing_erase (olsr_vid, olsrLink[my_pnum][n2]->did, OPC_ANIM_ERASE_MODE_XOR);
			olsrLink[my_pnum][n2]->did = -1;  // Reset did if link is erased
			//printf ("my drawing_erase: %d %d %d\n",my_pnum,n2,olsrLink[my_pnum][n2]->did);
			}
		}
	FOUT;
}
...
void Nrlolsr::OnPktCapture(smfT_olsr_ipc* ipc)
{
    ...
    {
#ifndef OPNET  // JPH 9/6/2006 - IPv6 not integrated with OPNET
		char* buffer;
...
/* JPH 12/6/06 - packet animation */

void olsr_col_erase()  // erase collision count drawing on behalf of dra_inoise_col pipeline stage
	{
	FIN(col_erase());
	op_anim_igp_drawing_erase (olsr_vid, olsrNode[my_pnum]->col_did, OPC_ANIM_ERASE_MODE_XOR);
	olsrNode[my_pnum]->col_did = -1;
	FOUT;
	}
...
extern "C"
{
	...
	VosT_Address _op_olsr_protolib_smf_alloc (VosT_Obtype, int);
	void olsr_protolib_smf (OP_SIM_CONTEXT_ARG_OPT)
		{
	...
	try
		...
		FSM_BLOCK_SWITCH
			...
			FSM_STATE_ENTER_FORCED (0, "proc_msg", state0_enter_exec, "olsr_protolib_smf [proc_msg enter execs]")
				...
				Packet* pkptr = op_pk_get (op_intrpt_strm ());
			...
			FSM_STATE_ENTER_UNFORCED (1, "init", state1_enter_exec, "olsr_protolib_smf [init enter execs]")
				...
				*/
				...
				if (!animInit && !no_node_anim)
					{
					animInit = OPC_TRUE;  // Animation initialization that is only done once.
					olsr_vid = op_anim_lprobe_anvid("olsr_anim");
					if (olsr_vid == OPC_ANIM_ANVID_NONE)
						printf("Warning: No animation probe with the label 'olsr_anim' has been defined.\n   Use DES/Choose Statistics (Advanced).\n");
					else if (olsr_vid == OPC_ANIM_ID_NIL)
						printf("Warning: No animation flow is active for 'olsr_anim'\n");
					else if (olsr_vid > 0)
						{
						printf("Animation Viewer ID: %d\n",olsr_vid);
						if (no_node_anim)
							{
							printf("Animation turned off by ANIM/Node Anim OFF global simulation attribute.\n");
							animFlag = OPC_FALSE;
							}
						else
							{
					...
					op_ima_sim_attr_get(OPC_IMA_STRING,"net_name",network_name);
					printf("Configure Animation for %s\n",network_name);
					op_anim_ime_nmod_draw(olsr_vid,OPC_ANIM_MODTYPE_NETWORK,network_name,"top.Campus Network",
							OPC_ANIM_MOD_OPTION_NONE,OPC_ANIM_DEFPROPS); 
					
					op_anim_igp_rect_fill (olsr_vid, OPC_ANIM_RETAIN | OPC_ANIM_COLOR_RGB222,
						" ", 0, 0, 2000, 3000);
					
					for (int i=0; i<MAX_NUM_NODES; i++)
						col_did[i] = -1;
						
					if (op_ima_sim_attr_exists("SDT flag"))
						op_ima_sim_attr_get(OPC_IMA_INTEGER,"SDT flag",&sdtFlag);
					if (sdtFlag)
						if (op_ima_sim_attr_exists("SDT filename"))
							{
							op_ima_sim_attr_get(OPC_IMA_STRING,"SDT filename",sdt_filename);
							sdtFile = fopen(sdt_filename, "w+");
							}
					if (sdtFile == NULL)
						{
						sdtFlag = OPC_FALSE;
						printf("No SDT file has been opened.  SDT script will not be created.\n");
						}
					else
						{
						printf("SDT file has been opened:  %s\n",sdt_filename);
						// Write initialization info to SDT script file.
						fprintf(sdtFile,"bgbounds 0,0,1000,1500");
						for (int i=0; i<12; i++)
							fprintf(sdtFile,"sprite %s image %s.png\n",icon_type[i],icon_type[i]);
						}
					}
					
				if (animFlag)
					{
					olsrNode[pnum] = new OlsrNode();
					olsrNode[pnum]->objid = own_node_objid;
					olsrNode[pnum]->olsr_objid = own_module_objid;
					olsrNode[pnum]->col_timeout_ev = op_ev_current();
					op_ima_obj_attr_get(olsrNode[pnum]->objid,"name",olsrNode[pnum]->name);
					
					op_anim_ime_nobj_update(olsr_vid,OPC_ANIM_OBJTYPE_NODE,my_node_name,
						OPC_ANIM_OBJ_ATTR_ICON,"blank",
						OPC_EOL);
					
					my_pnum = pnum;
					pnum++;
					op_ima_obj_state_set (own_node_objid, &my_pnum);
					}
				...
				if (op_intrpt_type () == OPC_INTRPT_REMOTE)
					...
					if (animFlag)
						for (int j=0; j<pnum; j++)
							if (my_pnum != j)
								olsrLink[my_pnum][j] = new OlsrLink();
			...
			FSM_TRANSIT_SWITCH
				...
				FSM_CASE_TRANSIT (5, 2, state2_enter_exec, olsr_col_erase();, "COL_TO", "olsr_col_erase()", "idle", "idle", "olsr_protolib_smf [idle -> idle : COL_TO / olsr_col_erase()]")
				FSM_CASE_TRANSIT (6, 2, state2_enter_exec, ;, "default", "", "idle", "idle", "olsr_protolib_smf [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
...
olsr_protolib_smf_state::operator new (size_t)
	...
	new_ptr = Vos_Alloc_Object (olsr_protolib_smf_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
VosT_Address
_op_olsr_protolib_smf_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
...
_op_olsr_protolib_smf_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	...
	if (strcmp ("mprOrder" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->mprOrder);
		FOUT
		}
	...
	if (strcmp ("node_name_did" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->node_name_did);
		FOUT
		}
	if (strcmp ("pk_collision_did" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->pk_collision_did);
		FOUT
		}
	if (strcmp ("my_node_name" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->my_node_name);
		FOUT
		}
	...
	if (strcmp ("no_link_anim" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->no_link_anim);
		FOUT
		}
	if (strcmp ("no_node_anim" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->no_node_anim);
		FOUT
		}
	*var_p_ptr = (void *)OPC_NIL;
